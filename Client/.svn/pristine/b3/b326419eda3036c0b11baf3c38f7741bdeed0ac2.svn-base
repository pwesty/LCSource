...
{	
	...
	switch(iNoticeType)
	{
	case Notice::NOTICE_NEWS_WEB:	// 웹 공지.
		{
			btnType = UBET_QUEST;
		}
		break;
	case Notice::NOTICE_NEWS_NORMAL:	// 공지 팝업
		{
			pData->SetNoticeBlink(iNoticeType, false);
			btnType = UBET_QUEST;
		}
		break;
	case Notice::NOTICE_QUEST:		// 퀘스트
	case Notice::NOTICE_EVENT:		// 이벤트
		btnType = UBET_QUEST;
		break;
	case Notice::NOTICE_POLYMOPH:	// 변신
		{
			pData->SetNoticeBlink(iNoticeType, false);
			btnType = UBET_ITEM;
		}		
		break;
	case Notice::NOTICE_AFFINITY:	// 친화도 보상
		{
			pData->SetNoticeBlink(iNoticeType, false);
			btnType = UBET_ACTION;
		}		
	...
	if (pVec != NULL)
	{
		...
		if (nMax > 0)
		{
			...
			if ( pIcon != NULL)
			{
				...
				CTString strTooltip;
				strTooltip.PrintF(_S(6423,"새로운 팝업 공지가 존재 합니다. 클립 하여 공지를 확인해 주세요"));
	...
	if (pVec != NULL)
	{
		...
		if (nMax > 0)
		{
			...
			if ( pIcon != NULL)
			{
				...
				CTString strTooltip;
				strTooltip.PrintF(_S(6422,"새로운 일반 공지가 존재 합니다. 클릭 하여 공지를 확인해 주세요"));
				pIcon->setTooltipWidth(DEF_TOOLTIP_WIDTH);
				pIcon->setTooltip(strTooltip.str_String);
	...
	if (pVec != NULL)
	{
		...
		if (nMax > 0)
		{
			...
			if ( pIcon != NULL)
			{
				...
				CTString strTooltip;
				strTooltip.PrintF(_S(4842,"친화도 알림"));
				pIcon->setTooltip(strTooltip.str_String);
	...
	if (pVec != NULL)
	{
		...
		if (nMax > 0)
		{
			...
			if ( pIcon != NULL)
			{
				...
				CTString strTooltip;
				strTooltip.PrintF(_S(5305, "변신 해제"));
				pIcon->setTooltip(strTooltip.str_String);
			...
			case QUEST_ONCE:
				{
					...
					strName.PrintF( _S( 559, "%s 퀘스트(1회만 수행가능)" ), QuestDD.GetName());
				}				
				break;
...
void CUINoticeGroup::SetGroup( GROUP_TYPE eType /*= QUEST_ONCE*/ )
{
	...
	if (m_pstrTitle != NULL)
	{
		...
		{
		case EVENT:
			strTitle.PrintF(_S(100, "이벤트"));
			break;
		case QUEST_ONCE:
			strTitle.PrintF(_S(99, "퀘스트"));
			break;
		case QUEST_UNLIMITED:
			strTitle.PrintF(_S(6274, "반복 퀘스트"));
			break;
		}
