...
: k_NAME c_string k_DEFAULT_POSE '{' matrix ';'
{
  ...
  be.be_OffSetLen = (FLOAT3D($5[3],$5[7],$5[11])).Length();
...
: k_NAME c_string ';' k_PARENT c_string ';' k_LENGTH float_const 
{
  ...
  CTString strBoneID = $2;
  CTString strParentBoneID = $5;
...
: c_string ';'
{
  ...
  CTString strTextureID = $1;
  if(strTextureID.Length()==0) {
    SurfShader.ss_spShaderParams.sp_aiTextureIDs[_ishParamIndex] = -1;
  } else {
    SurfShader.ss_spShaderParams.sp_aiTextureIDs[_ishParamIndex] = ska_StringToID($1);
  }
...
: triangle ';'
{
  ...
  mt.mt_uwVertexIndex[0]=$1[0];
  mt.mt_uwVertexIndex[1]=$1[1];
  mt.mt_uwVertexIndex[2]=$1[2];
  _iTriangle++;
}
;
