/*
 *  GfxMain.cpp - Gfx library implementation
 *  CTGfx graphics library
        ...
        BOOL bIfOpen/*=TRUE*/)
{
  ...
  {
    strRequestInDirectory = AfxGetApp()->GetProfileString("Scape", pchrRegistry, 
      _fnmApplicationPath+strDefaultDir);
  }
  ...
  ofnRequestFiles.hInstance = GetModuleHandle(ENGINEGUI_DLL_NAME);
  ofnRequestFiles.lpTemplateName = MAKEINTRESOURCE( IDD_GFX_FILE_REQUESTER);
  // allow multi select
  ...
  if( bIfOpen)
  {
    bResult = GetOpenFileName( &ofnRequestFiles);
  }
  else
  {
    bResult = GetSaveFileName( &ofnRequestFiles);
  }
  ...
  if( bResult)
  {
    ...
    if( pafnSelectedFiles != NULL)
    {
      ...
      if( pchrRegistry != NULL)
      {
        AfxGetApp()->WriteProfileString("Scape", pchrRegistry, chrFiles);
      }
    ...
    else
    {
      ...
      if( pchrRegistry != NULL)
      {
        AfxGetApp()->WriteProfileString("Scape", pchrRegistry, strChooseFilePath);
      }
