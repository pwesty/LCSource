...
//안태훈 수정 시작	//(Open beta)(2004-12-31)
FLOAT g_fAnimStrength = 0.5f;
//안태훈 수정 끝	//(Open beta)(2004-12-31)
...
//안태훈 수정 시작	//(Ska Shader Configue Panel Size Fix)(0.1)
#define YRESIZE_BY_ANT	160
//안태훈 수정 끝	//(Ska Shader Configue Panel Size Fix)(0.1)
...
/* !
 * Child 모델을 RideSetting을 위한 Offset설정으로 전환하면서, Tree메뉴에 추가한다.
 * 펫(or 일반 모델엔티티)에 탑승을 위한 처리를 child모델의 Offset값으로 사용
 * 탑승을 위해서는 Child모델데이터를 함께 저장할 필요가 없다.
*/
void CDlgBarTreeView::AddRideSetting(CModelInstance &mi,HTREEITEM hParent)
{
	/// Offset 및 Parent BONE 정보만 취하고, Child모델은 삭제한다.
	// add all frames bounding box
