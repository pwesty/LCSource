...
#define COLUMN_P 11
#define COLUMN_B 12
#define COLUMN_MOBINDEX	13
#define COLUMN_REGEN	14
#define COLUMN_RAIDOBJ	15
#define COLUMN_PROPERTY_START 16
...
CTString GetPropertyValue(CEntity *pen, CEntityProperty *pepProperty, INDEX &iFormat)
{
  ...
  {
    strResult.PrintF("0x%08x", ENTITYPROPERTY( pen, pepProperty->ep_slOffset, ULONG64));
    break;
  }
...
BEGIN_MESSAGE_MAP(CDlgBrowseByClass, CDialog)
	...
	ON_BN_CLICKED(ID_EXPORT_BROWSE_BY_CLASS, OnExportBrowseByClass)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
void CDlgBrowseByClass::InitializeListColumns(void)
{
  ...
  PIX pixEntityID         = rectListControl.Width()*ID_PERCENTAGE/100;
  ...
  m_listEntities.InsertColumn( COLUMN_ID, "ID", LVCFMT_LEFT, pixEntityID);
  ...
  PIX pixPlacement = (rectListControl.Width()-
    (pixEntityID+pixEntityClassName+pixInstanceName+
...
// 강동민 수정 시작
	m_listEntities.InsertColumn( COLUMN_MOBINDEX, "NPCTYPE", LVCFMT_LEFT, pixPlacement);
	m_listEntities.InsertColumn( COLUMN_REGEN, "REGENTIME", LVCFMT_LEFT, pixPlacement);
	m_listEntities.InsertColumn( COLUMN_RAIDOBJ, "RaidObject", LVCFMT_LEFT, pixPlacement);
// 강동민 수정 끝
    ...
    for(;pdecDLLClass!=NULL; pdecDLLClass = pdecDLLClass->dec_pdecBase)
    {
      ...
      for(INDEX iProperty=0; iProperty<pdecDLLClass->dec_ctProperties; iProperty++)
      {
        ...
        if( pepProperty->ep_strName!=CTString(""))
        {
          ...
          m_listEntities.InsertColumn( COLUMN_PROPERTY_START+_ctProperties, pepProperty->ep_strName, LVCFMT_LEFT, pixProperty);
          _ctProperties++;
        }
      }
    }
...
BOOL CDlgBrowseByClass::OnInitDialog() 
{
  ...
  PIX pixButtonOccupies = (iScreenX-pixSX*2)/10;
...
#define MOVE_BUTTON( id, button_collumn)\
  ...
  MOVE_BUTTON( ID_EXPORT_BROWSE_BY_CLASS, 10);
  ...
  if( pNMListView->iSubItem == COLUMN_DISTANCE)
  {
    ...
    if( iSelectedItem != -1)
    {
      ...
      for( INDEX iItem = 0; iItem<m_listEntities.GetItemCount(); iItem++) 
      {
        ...
        m_listEntities.SetItemText( iItem, COLUMN_DISTANCE, achrNumber);
      }
    }
    else
    {
...
void CDlgBrowseByClass::OnLeave() 
{
  ...
  m_listEntities.SetRedraw(FALSE);
  INDEX ctItems = m_listEntities.GetItemCount();
  for (iItem = 0; iItem<ctItems; iItem++) 
  {
...
BOOL CDlgBrowseByClass::AddRaidEntityData()
{
	CDynamicContainer<class CEntity> *penContainer = GetCurrentContainer();
	CStaticStackArray<RAIDENTITYDATA> RaidEnityList;
	...
	{FOREACHINDYNAMICCONTAINER(*penContainer, CEntity, iten)
		{
			// for all non-hidden entities that are not classified into hidden sectors, filter importants if requested
			CBrushSector *pbscSector = iten->GetFirstSector();
			if (!(iten->en_ulFlags&ENF_HIDDEN) && ((pbscSector == NULL) || !(pbscSector->bsc_ulFlags & BSCF_HIDDEN)) &&
				(!m_bShowImportants || iten->IsImportant()) /*&& iten->GetRaidObject()*/) // 레이드 오브젝트로 설정된 Entity
			{
				// 1. 클래스 이름
				CEntityClass *pecEntityClass = iten->GetClass();
				CDLLEntityClass *pdllecDllEntityClass = pecEntityClass->ec_pdecDLLClass;
				RAIDENTITYDATA tmpEntityData;
				tmpEntityData.strClassName = pdllecDllEntityClass->dec_strName;
				tmpEntityData.EntityID = iten->en_ulID;
				tmpEntityData.strName = iten->GetName();
				tmpEntityData.Position = iten->GetPlacement().pl_PositionVector;
				tmpEntityData.ulEventType = iten->GetRaidEvent();

				RaidEnityList.Add(tmpEntityData);
			}
		}
	}

	if (RaidEnityList.Count() > 0)
	{
		//m_queBuffer.WriteDword((DWORD)RaidEnityList.Count());
		int i;
		for (i=0; i<RaidEnityList.Count(); ++i)
		{
			CString strFlush;
			strFlush.Format(_T("%s,%d,%s,%f,%f,%f,%d\n"),
				RaidEnityList[i].strClassName.str_String,	// 클래스 이름
				RaidEnityList[i].EntityID,	// Entity Unique Index
				RaidEnityList[i].strName.str_String, // Entity 이름
				RaidEnityList[i].Position(1), RaidEnityList[i].Position(2), RaidEnityList[i].Position(3), // 위치
				RaidEnityList[i].ulEventType); // 이벤트 번호

			m_queBuffer.WriteDocString(strFlush.GetBuffer(0));
			//m_queBuffer.WriteString(strFlush.GetBuffer(0));
/*			m_queBuffer.WriteString(RaidEnityList[i].strClassName.str_String);
			m_queBuffer.WriteDword(RaidEnityList[i].EntityID);
			m_queBuffer.WriteString(RaidEnityList[i].strName.str_String);
			m_queBuffer.WriteFloat(RaidEnityList[i].Position(1));
			m_queBuffer.WriteFloat(RaidEnityList[i].Position(2));
			m_queBuffer.WriteFloat(RaidEnityList[i].Position(3));
			m_queBuffer.WriteDword(RaidEnityList[i].ulEventType);*/
		}
...
void CDlgBrowseByClass::OnExportBrowseByClass() 
{
	// TODO: Add your control notification handler code here
/*	CFileDialog FileDlg(TRUE, _T("eed"), _T("*.eed"), OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, 
		_T("eed File(*.eed)|*.eed|ALL Files (*.*)|*.*|"));*/
	CFileDialog FileDlg(TRUE, _T("csv"), _T("*.csv"), OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, 
	...
	if (IDOK == FileDlg.DoModal())
	{
		...
		File.open(FileDlg.GetPathName(), ios::out | /*ios::binary |*/ ios::trunc);
		...
		if (AddRaidEntityData()) // RaidData를 큐에 담는다.
		{
