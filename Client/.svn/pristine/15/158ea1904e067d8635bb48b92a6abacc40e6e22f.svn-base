...
BOOL CBrushShadowMap::IsShadowFlat( COLOR &colFlat)
{
	...
	col = pbpo->bpo_pbscSector->bsc_colAmbient;
	...
	{ 
		...
		if( ls.ls_ulFlags&LSF_DIRECTIONAL)
		{
			...
			if( !(pbpo->bpo_ulFlags&BPOF_NOPLANEDIFFUSION)) {
				...
				const FLOAT fIntensity = (pbpo->bpo_pbplPlane->bpl_plAbsolute) % vLightDirection; // ! inverted
			...
			if( pbpo->bpo_ulFlags&BPOF_HASDIRECTIONALAMBIENT) {
				...
				ASSERT( ls.ls_fAmbientAttenuation>=0.0f && ls.ls_fAmbientAttenuation<=1.0f);
				col = ls.GetLightAmbient();
				if( ls.ls_fAmbientAttenuation>0.01f) { // apply attenuation if allowed
					fAmbientIntensity = 1.0f + ls.ls_fAmbientAttenuation*(fAmbientIntensity-1.0f);
					...
					col = MulColors( col, ulAmbientIntensity);
				}
			...
			col = MulColors( ls.GetLightColor(), ulLightIntensity);
			ColorToRGB( col, ubR,ubG,ubB);
			slR += ubR;  slG += ubG;  slB += ubB; 
