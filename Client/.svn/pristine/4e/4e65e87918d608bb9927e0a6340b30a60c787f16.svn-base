...
void CDlgSelectMode::DoDataExchange(CDataExchange* pDX)
{
  ...
  { 
    ...
    { // prepare resoultion string
      ...
      // if not yet added
      if( m_ctrlResCombo.FindStringExact( 0, strRes) == CB_ERR) {
        // add it to combo box list
        i = m_ctrlResCombo.AddString(strRes);
...
//안태훈 수정 끝	//(Modify Default API to D3D)(0.1)
  ...
  // if screen or window opening was not succesefull
  if( pViewPort == NULL) {
    AfxMessageBox( "Unable to setup full screen display. Test mode failed.");
    return;
  }
  ...
  if( AfxMessageBox( "Did You see displayed message corectly?", MB_YESNO) == IDYES) {
