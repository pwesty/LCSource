...
event EStartSounds {  // these two are used in network
};
event EStopSounds {
};
event EBrushDeath { // sent by the entity to itself to sync death over net
...
//안태훈 수정 시작	//(Add Sun Moon Entity and etc)(0.1)
 99 BOOL m_bStartAtFirstMarker    "Start At First Marker" = FALSE,	//TRUE면 시작시 첫번째 Marker위치에서 시작
//안태훈 수정 끝	//(Add Sun Moon Entity and etc)(0.1)
 ...
 100 BOOL m_bFade				"Fade Debris"		= TRUE,
 101 BOOL m_bCollision			"Collision Debris"	= FALSE,
...
functions:
 ...
 void Precache(void)
		...
		FLOAT fDamageAmmount, const FLOAT3D &vHitPoint, const FLOAT3D &vDirection) 
	{
		...
		if (m_bBlowupByBull)
		{
			...
			if( dmtType==DMT_IMPACT)
				//&& IsOfClass( penInflictor, &CWerebull_DLLClass)) 
			{
	...
	BOOL LoadMarkerParameters()
	{
		...
		// speed
//안태훈 수정 시작	//(Add Sun Moon Entity and etc)(0.1)
		//if (mbm.m_fSpeed > 0.0f) { m_fSpeed = mbm.m_fSpeed; }	//원본
