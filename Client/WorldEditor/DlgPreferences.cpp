...
void CDlgPreferences::DoDataExchange(CDataExchange* pDX)
{
  ...
  if( pDX->m_bSaveAndValidate != FALSE)
  {
    ...
    _pShell->SetFLOAT("mth_fCSGEpsilon", pow(2.f, (m_ctrlCSGPrecission.GetPos() - 5) * 2)); 
    theApp.m_Preferences.ap_iStartupWindowSetup = m_iWndStartupCfg;
    theApp.m_Preferences.ap_strSourceSafeProject = m_strSSProject;
...
BOOL CDlgPreferences::OnInitDialog()
{
  ...
  {
    INDEX iAddedAs=m_ctrGfxApi.AddString("OpenGL");
