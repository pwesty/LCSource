...
void CDlgWorldSettings::DoDataExchange(CDataExchange* pDX)
{
  ...
  {
    CWorldEditorDoc *pDoc = theApp.GetDocument();
    pDoc->m_woWorld.wo_strBackdropUp = m_strTopViewPicture;
    ...
    pDoc->m_woWorld.wo_strBackdropFt = m_strFrontViewPicture;
    ...
    pDoc->m_woWorld.wo_strBackdropRt = m_strRightViewPicture;
    ...
    pDoc->m_woWorld.wo_strBackdropObject = m_strBackdropObject;
    ...
    _strupr( chrColor);
    theApp.WriteProfileString( "World editor prefs", "Default background color", chrColor);

    pDoc->m_woWorld.SetName( CTString(m_strLevelName));
...
void CDlgWorldSettings::SetupBcgSettings( BOOL bOnNewDocument)
{
  ...
  if( bOnNewDocument)
  {
    ...
    m_fnBackgroundPicture = "Data\\Textures\\Editor\\Default.tex";
...
void CDlgWorldSettings::OnBrowseTopViewPicture()
{
  ...
  if( fnPicture == "") return;
  GetDlgItem( IDC_TOP_VIEW_PICTURE_T)->SetWindowText( fnPicture);
	m_strTopViewPicture = fnPicture;
  CWorldEditorDoc *pDoc = theApp.GetDocument();
  pDoc->SetupBackdropTextureObject( CTString(m_strTopViewPicture), pDoc->m_toBackdropUp);
}
...
void CDlgWorldSettings::OnBrowseFrontViewPicture()
{
  CTFileName fnPicture = _EngineGUI.FileRequester( "Picture for front view",
    FILTER_PICTURES FILTER_ALL FILTER_END, "Picture for view directory", "");
  if( fnPicture == "") return;
  GetDlgItem( IDC_FRONT_VIEW_PICTURE_T)->SetWindowText( fnPicture);
	m_strFrontViewPicture = fnPicture;
  CWorldEditorDoc *pDoc = theApp.GetDocument();
  pDoc->SetupBackdropTextureObject( CTString(m_strFrontViewPicture), pDoc->m_toBackdropFt);
}
...
void CDlgWorldSettings::OnBrowseRightViewPicture()
{
  CTFileName fnPicture = _EngineGUI.FileRequester( "Picture for right view",
    FILTER_PICTURES FILTER_ALL FILTER_END, "Picture for view directory", "");
  if( fnPicture == "") return;
  GetDlgItem( IDC_RIGHT_VIEW_PICTURE_T)->SetWindowText( fnPicture);
	m_strRightViewPicture = fnPicture;
  CWorldEditorDoc *pDoc = theApp.GetDocument();
  pDoc->SetupBackdropTextureObject( CTString(m_strRightViewPicture), pDoc->m_toBackdropRt);
}
...
void CDlgWorldSettings::OnBrowseBackdropObject()
{
  ...
  if( fnObject == "") return;
  GetDlgItem( IDC_BACKDROP_OBJECT_T)->SetWindowText( fnObject);
