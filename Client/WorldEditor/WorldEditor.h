...
extern CTextureData*	GetTempColorMap( INDEX iShadow );			// yjpark |<--
extern CTextureData*	GetTempShadowMap( INDEX iTempShadowMap );
extern CTextureData*	GetShadowMap( INDEX iShadowMap );
extern CTextureData*	GetShadowMap();
extern INDEX			GetShadowMapIndex();
extern void				SelectShadowMap( INDEX iShadowMap );		// yjpark      -->|

#define ALLOW_TERRAINS 1

#define MAX_COUNTRY		21
#define MALAYSIA_ENG	8
#define HONGKONG_ENG	12

// Country
static CString _aStrCountry[MAX_COUNTRY] =
{	
	_T("국내"),
	_T("혁명"),
	_T("천하대란"),
...
//강동민 수정 시작 테스트 클라이언트 작업	06.10
	// 내부에서만 사용되는 구조체.
	typedef struct __tagDBServerInfo
	{
		char name[100];
		char ip[20];
		char uid[40];
	...
	typedef struct __tagDBServerList
	{
		int n;
		DBServerInfo* server;
	} DBServerList;
...
public:
	...
	INDEX m_iApi;
	CTFileName m_fnSSPath;   // source safe's path
	...
	// Prev rendering preferences for each view		:Su-won
	CViewPrefs m_vpPrevViewPrefs[10][4];
	...
	void ConvertList(const CTFileName &fnmList);
...
//강동민 수정 시작 테스트 클라이언트 작업	06.10
	...
	// DB 서버와 관련된 작업.
	//----------------------------------------------------
	BOOL			GetServerList(const char* FileName);
	BOOL			ConnectToDBServer(int iIndex);
//강동민 수정 끝 테스트 클라이언트 작업		06.10
...
#include "DlgPgAttribute.h"				// yjpark
#include "DlgExportBspAttributeMap.h"	// yjpark
#include "DlgExportBspHeightMap.h"		// yjpark
