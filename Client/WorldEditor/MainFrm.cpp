...
#define GET_COLOR_FROM_INI(iColor, strColorIndex) \
	...
	strcpy( chrColor, theApp.GetProfileString( "Custom picker colors", "Color" strColorIndex, chrColor));\
...
#define SET_COLOR_TO_INI(iColor, strColorIndex) \
	{char chrColor[ 16];\
	sprintf( chrColor, "0x%08x", acol_ColorizePallete[iColor]);\
	theApp.WriteProfileString( "Custom picker colors", "Color" strColorIndex, chrColor);}
...
BEGIN_MESSAGE_MAP(CMainFrame, CMDIFrameWnd)
	...
	ON_COMMAND(ID_NPC_OPEN, OnNpcOpen)
	ON_COMMAND(ID_MOB_OPEN, OnMobOpen)
	ON_COMMAND(ID_CONNECT_DB, OnConnectDb)
	ON_UPDATE_COMMAND_UI(ID_CONNECT_DB, OnUpdateConnectDb)
	ON_UPDATE_COMMAND_UI(ID_MOB_OPEN, OnUpdateMobOpen)
	ON_UPDATE_COMMAND_UI(ID_NPC_OPEN, OnUpdateNpcOpen)
	ON_COMMAND(ID_NUMOFENEMY, OnNumofenemy)
	ON_COMMAND(ID_BACKUP_DATA, OnBackupData)
	ON_UPDATE_COMMAND_UI(ID_BACKUP_DATA, OnUpdateBackupData)
	ON_COMMAND(ID_RESTORE_DATA, OnRestoreData)
	ON_UPDATE_COMMAND_UI(ID_RESTORE_DATA, OnUpdateRestoreData)
	ON_UPDATE_COMMAND_UI(ID_VIEW_BLOOM, OnUpdateViewBloom)
	ON_COMMAND(ID_VIEW_BLOOM, OnViewBloom)
	ON_COMMAND(ID_VIEW_TRANSLUCENT, OnViewTranslucent)
	ON_UPDATE_COMMAND_UI(ID_VIEW_TRANSLUCENT, OnUpdateViewTranslucent)
	ON_COMMAND(ID_VIEW_INVISIBLEOFF, OnViewInvisibleoff)
	ON_UPDATE_COMMAND_UI(ID_VIEW_INVISIBLEOFF, OnUpdateViewInvisibleoff)
	ON_COMMAND(ID_VIEW_SHOWPORTAL, OnViewShowportal)
	ON_UPDATE_COMMAND_UI(ID_VIEW_SHOWPORTAL, OnUpdateViewShowportal)
	ON_COMMAND(ID_INVISIBLE_TOGGLE, OnInvisibleToggle)
	//}}AFX_MSG_MAP
...
CMainFrame::CMainFrame()
{
	...
	m_pwndToolTip = NULL;
	m_pNpcManagerDialog = NULL;
	m_pMobDataDialog = NULL;
}
...
CMainFrame::~CMainFrame()
{
	...
	pApp->WriteProfileString("World editor", "Last virtual tree", (CTString&)(m_fnLastVirtualTree));
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_fnLastVirtualTree = CTString( pApp->GetProfileString("World editor",
	...
	// We will set deafault width and height of browser and property dialog bars
...
void CMainFrame::OnViewInfowindow()
{
	...
	if( m_pInfoFrame == NULL)
	{
		...
		if( !m_pInfoFrame->Create( NULL, "Tools info",
				MFS_SYNCACTIVE|WS_POPUP|WS_CAPTION|WS_SYSMENU|MFS_THICKFRAME, rectInfoWindow, this))
	  {
...
void CMainFrame::StartApplication( CTString strApplicationToRun)
{
	...
	// here we will recieve result of process creation
	...
	// if process creation was not successefull
	if( !bSuccess)
	{
...
{
	// obtain stop time
	m_wndStatusBar.SetPaneText( iPane, strMessage, TRUE);
	FLOAT tmNow = _pTimer->GetHighPrecisionTimer().GetSeconds();
	theApp.m_tmStartStatusLineInfo=tmNow+fTime;
}

//강동민 수정 시작 접속 시퀀스 작업	05.27
void CMainFrame::OnNpcOpen() 
{
	if(theApp.m_iWorldNum	== -1)
	{
		AfxMessageBox(_T("월드 정보 화일을 먼저 오픈하십시오!"), MB_OK);
		return;
	}
	// TODO: Add your command handler code here
	if(m_pNpcManagerDialog)
	{
		m_pNpcManagerDialog->SetFocus();
		m_pNpcManagerDialog->ShowWindow(SW_SHOW);
	}
	else
	{
		m_pNpcManagerDialog = new CNpcManagerDialog;
		m_pNpcManagerDialog->Create(IDD_NPC_MANAGER, this);	
		m_pNpcManagerDialog->ShowWindow(SW_SHOW);
	}	
}

void CMainFrame::OnMobOpen() 
{
	if(theApp.m_iWorldNum	== -1)
	{
		AfxMessageBox(_T("월드 정보 화일을 먼저 오픈하십시오!"), MB_OK);
	...
	if(m_pMobDataDialog)
	{
		m_pMobDataDialog->SetFocus();
		m_pMobDataDialog->ShowWindow(SW_SHOW);
	}
	else
	{
		m_pMobDataDialog = new CMobDataDialog;
		m_pMobDataDialog->Create(IDD_MOB_DATA_DIALOG, this);
		m_pMobDataDialog->ShowWindow(SW_SHOW);
	}	
}
//강동민 수정 끝 접속 시퀀스 작업	05.27
...
void CMainFrame::OnConnectDb() 
{
	...
	fnTemp					= strFullPath + "serverlist.txt";

	if(!theApp.GetServerList(fnTemp))	return;

	const int i = 0;
	theApp.m_bDBConnected	= theApp.ConnectToDBServer(i);
	if(!theApp.m_bDBConnected)
	{
		AfxMessageBox(_T("DB에 접속할 수 없습니다."));
		return;
	}
}

void CMainFrame::OnUpdateConnectDb(CCmdUI* pCmdUI) 
{
	// TODO: Add your command update UI handler code here
	pCmdUI->Enable(!theApp.m_bDBConnected);
}

void CMainFrame::OnUpdateMobOpen(CCmdUI* pCmdUI) 
{
	// TODO: Add your command update UI handler code here
	pCmdUI->Enable(theApp.m_bDBConnected);	
}

void CMainFrame::OnUpdateNpcOpen(CCmdUI* pCmdUI) 
{
	// TODO: Add your command update UI handler code here
	pCmdUI->Enable(theApp.m_bDBConnected);	
}
...
void CMainFrame::OnNumofenemy() 
{
	// TODO: Add your command handler code here
	CString strMessage = _T("");
	int iNumOfEnemies = theApp.GetNumOfEnemies();
	strMessage.Format(_T("전체 Enemy Spawner로 부터 생성되는 에너미는 총 %d 마리입니다."), iNumOfEnemies);
...
void CMainFrame::OnBackupData() 
{
	...
	if(m_pNpcManagerDialog)
	{
		m_pNpcManagerDialog->m_pNpcDlg->RestoreData();
	}
}

void CMainFrame::OnUpdateBackupData(CCmdUI* pCmdUI) 
{
	// TODO: Add your command update UI handler code here
	pCmdUI->Enable(theApp.m_bDBConnected);	
}
...
void CMainFrame::OnRestoreData() 
{
	...
	if(m_pNpcManagerDialog)
	{
		m_pNpcManagerDialog->m_pNpcDlg->BackupData();
	}	
}
...
void CMainFrame::OnViewBloom() 
{
	// TODO: Add your command handler code here
	_bUseBloomInWorldEditor =!_bUseBloomInWorldEditor;

	theApp.RefreshAllDocuments();	
}

void CMainFrame::OnUpdateViewBloom(CCmdUI* pCmdUI) 
{
	// TODO: Add your command update UI handler code here
	pCmdUI->SetCheck( _bUseBloomInWorldEditor);	
}

void CMainFrame::OnViewTranslucent() 
{
	// TODO: Add your command handler code here
	_bTranslucentModel =!_bTranslucentModel;

	theApp.RefreshAllDocuments();	
	
}

void CMainFrame::OnUpdateViewTranslucent(CCmdUI* pCmdUI) 
{
	// TODO: Add your command update UI handler code here
	pCmdUI->SetCheck( _bTranslucentModel);	
}

void CMainFrame::OnViewInvisibleoff() 
{
	// TODO: Add your command handler code here
	_bInvisibleOff =!_bInvisibleOff;

	theApp.RefreshAllDocuments();
	
}

void CMainFrame::OnUpdateViewInvisibleoff(CCmdUI* pCmdUI) 
{
	// TODO: Add your command update UI handler code here
	pCmdUI->SetCheck( _bInvisibleOff);		
}

void CMainFrame::OnViewShowportal() 
{
	// TODO: Add your command handler code here
	_bShowPortalPolygon =!_bShowPortalPolygon;

	theApp.RefreshAllDocuments();
	
}

void CMainFrame::OnUpdateViewShowportal(CCmdUI* pCmdUI) 
{
	// TODO: Add your command update UI handler code here
	pCmdUI->SetCheck( _bShowPortalPolygon);	
}

void CMainFrame::OnInvisibleToggle() 
{
	// TODO: Add your command handler code here
	_bInvisibleOff =!_bInvisibleOff;	
}
