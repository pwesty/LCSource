...
public:
	...
	inline CActiveEdge(void) : ace_psedEdge(NULL) {};
...
#define DMF_MIRROR   (1UL<<4)  // in mirrored/warped projection
...
//강동민 수정 시작
	/* Render a ska model. */
	// SKA 모델을 렌더링합니다.
	//void CRenderer::RenderOneSkaModel( CEntity &en, const CPlacement3D &plModel,	// 원본.
	void RenderOneSkaModel( CEntity &en, const CPlacement3D &plModel, 
		BOOL bRenderShadow, FLOAT fDMDistance, ULONG ulDMFlags);
	
	// NOTE : 추후 캐릭터나 몬스터의 모습을 윈도우 등에 보여주는 식으로 사용할수 있을듯...
...
//강동민 수정 끝 버그 사냥 작업		09.09
	...
	HRESULT CreateVShTexCoord4Offset( LPDIRECT3DDEVICE8 pd3dDevice, 
                                    DWORD* pdwVertexDecl );
	// SRS Settings
