/*
 *  RenCache.cpp - Cache functions for renderer
...
void CRenderer::PostClipVertices(void)
{
	...
	if (re_pbrCurrent->br_prProjection.IsPerspective()) {
		...
		for(INDEX ivx = re_iViewVx0; ivx<iVxTop; ivx++) {
			...
			if ( vvx.vvx_vView(3) > 0.0f || vvx.vvx_vView(3) < 0.0f )
			{
				fooz = 1.0f/vvx.vvx_vView(3);
			}
 ...
 */
inline void CRenderer::MakeScreenEdge( CScreenEdge &sed, FLOAT fI0, FLOAT fJ0, FLOAT fI1, FLOAT fJ1)
{
...
CScreenPolygon *CRenderer::MakeScreenPolygon(CBrushPolygon &bpo)
{
	...
	BOOL	bSelected = FALSE;	// yjpark |<--

#ifdef	WORLD_EDITOR
	extern BOOL	_bShowPolygonAttribute;		// yjpark
	// if the polygon is selected
	bSelected = PolygonIsSelected( bpo, br, bsc);
	if (bSelected) {
		// mark this polygon for drawing as selected
		sppo.spo_ulFlags |= SPOF_SELECTED;
	}
	// If the polygon has the attribute				// yjpark |<--
	else if( _bShowPolygonAttribute && bpo.bpo_ubPolygonAttribute > BPOA_NONE )
	{
		sppo.spo_ulFlags |= SPOF_ATTRIBUTE;
		if(  bpo.bpo_ubPolygonAttribute > BPOA__WAR5F )						// Free PK Zone
		{
			sppo.spo_ulFlags |= SPOF_FREEPK_ZONE;
		}
		else if(  bpo.bpo_ubPolygonAttribute > BPOA_PRODUCTZONE_PRIVATE5F )		// War Area
		{
			sppo.spo_ulFlags |= SPOF_ATTWAR_AREA;
		}
		else if(  bpo.bpo_ubPolygonAttribute > BPOA_PRODUCTZONE_PUBLIC5F )		// Production zone - private
		{
			sppo.spo_ulFlags |= SPOF_ATTPRODUCT_ZONE_PRIVATE;
		}
		else if(  bpo.bpo_ubPolygonAttribute > BPOA_PEACEZONE5F )			// Production zone - private
		{
			sppo.spo_ulFlags |= SPOF_ATTPRODUCT_ZONE_PUBLIC;
		}
		else if(  bpo.bpo_ubPolygonAttribute > BPOA_STAIRWALL4F5F )			// Peace zone
		{
			sppo.spo_ulFlags |= SPOF_ATTPEACE_ZONE;
		}
		else if(  bpo.bpo_ubPolygonAttribute > BPOA_STAIR4F5F )				// Wall of stair
		{
			sppo.spo_ulFlags |= SPOF_ATTSTAIRWALL;
		}
		else if( bpo.bpo_ubPolygonAttribute > BPOA_UNWALKABLE5F )			// Stair
		{
			sppo.spo_ulFlags |= SPOF_ATTSTAIR;
		}		
		else
		{
			BOOL	bOdd = bpo.bpo_ubPolygonAttribute % 2;
			
			if( bpo.bpo_ubPolygonAttribute > BPOA_WALKABLE5F )			// Unwalkable
			{
				if( bOdd )
					sppo.spo_ulFlags |= SPOF_ATTUNWALKABLE_ODD;
				else
					sppo.spo_ulFlags |= SPOF_ATTUNWALKABLE_EVEN;
			}
			else															// Walkable
			{
				if( bOdd )
					sppo.spo_ulFlags |= SPOF_ATTWALKABLE_ODD;
				else
					sppo.spo_ulFlags |= SPOF_ATTWALKABLE_EVEN;
			}
		}
	}													// yjpark     -->|
...
void CRenderer::AddPolygonToScene( CScreenPolygon *pspo)
{
	...
	GFXVertex *pvtx   = _avtxScene.Push(ctVtx);
	...
	if (sppo.spo_ctElements>0) {
		sppo.spo_piElements = &bpo.bpo_aiTriangleElements[0];
	}
	_sfStats.IncrementCounter( CStatForm::SCI_SCENE_TRIANGLES, sppo.spo_ctElements);
}
