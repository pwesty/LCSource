...
#elif defined (LC_ESP_USA)
#elif defined (LC_FRA_USA)
#elif defined (LC_NLD)
#else
...
#ifdef LC_USA
#define PULSE_CONNECT_TIMEOUT		(6 * PULSE_REAL_MIN)	// 입력 대기 시간, 지정 시간 동안 입력이 없으면 접속을 종료
#else
#define PULSE_CONNECT_TIMEOUT		(3 * PULSE_REAL_MIN)	// 입력 대기 시간, 지정 시간 동안 입력이 없으면 접속을 종료
#endif // LC_USA

#if defined (LC_GER) || defined (LC_PLD) || defined(LC_SPN) || defined(LC_FRC) || defined(LC_ITA) || defined(LC_TUR)
#define PULSE_SAVE_PC				(3 * PULSE_REAL_MIN)	// 자동 저장 시간
#else // LC_GAMIGO
...
#ifdef TRIPLE_DES
#define MAX_PWD_LENGTH				64		// 암호 길이
#else
...
#else
#define MAX_STASH_PASSWORD_LENGTH	14		// 창고 암호 길이
#endif
...
#define HAIR_NIGHTSHADOW	3
#endif // #ifdef SYSTEM_FACEOFF
...
// 게임 로그인 종류
#if defined (TEST_SERVER) || defined (IMP_ADD_CHAR_SLOT)
...
//#define	MAX_MEMPOS_PRIMIUM			20			// 장소 기억수
...
#define		NPC1_INSTANT			(1 << 1)	// 레이드 인스턴트 던전에 나오는 NPC
...
#define		NPC1_TOTEM_NPC			(1 << 6)	// 토템 npc는 공격을 받지 않는다.
...
#define		ITEM_FLAG_ABS		(1 << 16)	// 시간제 아이템
#define		ITEM_FLAG_NOTREFORM	(1 << 17)	// 리폼 불가 아이템
//#define     ITEM_FLAG_CAO		(1 << 18)	// 카오전용아이템
#define		ITEM_FLAG_ZONEMOVE_DEL	(1 << 18) // 존이동 시 아이템 삭제
#define     ITEM_FLAG_ORIGIN	(1 << 19)	// 오리지널 옵션
#define     ITEM_FLAG_TRIGGER	(1 << 20)	// 트리거 이벤트에 사용되는 아이템
#define     ITEM_FLAG_RAIDSPECIAL (1 << 21)	// 현재 사용 안함(추후 다른 것으로 사용)
#define		ITEM_FLAG_QUEST		(1 << 22) // 퀘스트
#define		ITEM_FLAG_BOX		(1 << 23) //  럭키드로우박스
#define		ITEM_FLAG_NOTTRADEAGENT	(1 << 24)	// 거래대행등록 불가
#define     ITEM_FLAG_QUEST_TRIGGER	(1 << 25)	// 퀘스트 트리거
#define		ITEM_FLAG_COSTUME2	(1 << 26)
#define		ITEM_FLAG_SOCKET	(1 << 27)		// 소켓 생성 가능 아이템
#define		ITEM_FLAG_SELLER	(1 << 28)		// 상인용 아이템
#define		ITEM_FLAG_CASTLLAN	(1 << 29)		// 성주만 착용 가능한 아이템
#define		ITEM_FLAG_LETSPARTY (1 << 30)		// 파티형아이템 ( 파티에 메세지 전송 )
#define		ITEM_FLAG_DECOMPOSE (1 << 31)		// 분해가능한 아이템.
...
// #ifdef REFORMER_SYSTEM
#define		IETC_REFORMER		21
// #endif
//ifdef SOCKET_SYSTEM_S2
...
#else
#define ITEM_UPGRADE_GENERAL_MAXLEVEL		150
#endif
...
// 착용 관련
#ifdef COSTUME2_SYSTEM
#define MAX_WEARING					28	// 캐릭터 당 최대 착용 장비 수 : 투구 상의 무기 하의 방패 장갑 신발 악세1 악세2 악세3 애완동물 등
#elif defined (ENABLE_BACKWING)
#define MAX_WEARING					12	// 캐릭터 당 최대 착용 장비 수 : 투구 상의 무기 하의 방패 장갑 신발 악세1 악세2 악세3 애완동물 등
#else //if defined(ENABLE_PET)
...
//#if defined(NEW_PK) || defined( ADULT_SERVER )
#define		RAMODE_USE_PULSE			(3 * PULSE_REAL_MIN)	// 정당방위 유지 시간 30min
//#else
//#define		RAMODE_USE_PULSE			(30 * PULSE_REAL_SEC)	// 정당방위 유지 시간 30s
//#endif // NEW_PK || ADULT_SERVER
...
#ifdef SUB_HELPER
#define IS_RUNNING_SUBHELPER		(!gserver.m_brunsubhelper && gserver.m_subHelper && !gserver.m_subHelper->m_bclosed)
#endif //SUB_HELPER

#define IS_RUNNING_LCSOAPHELPER		(!gserver.m_brunsoaphelper && gserver.m_soaphelper && !gserver.m_soaphelper->m_bclosed)
...
#define		OPTION_INCREASE_INVEN		50		// 소지량 증가
...
//
	...
	#define		TEACH_LIMIT_DAY				1			// 테스트 서버에서는 1일 동안 견습생이 접속을 않하면 사제 관계를 끊는다.
...
#if defined(TEST_SERVER) || defined(LC_KOR) || defined(LC_USA) || defined(LC_JPN) || defined (LC_GAMIGO) || defined(LC_BILA) || defined(LC_RUS) || defined(LC_TLD)
#define SUMMON_DELAY			(PULSE_REAL_SEC * 5)	// 소환 딜레이
#else
#define SUMMON_DELAY			(PULSE_REAL_MIN * 5)	// 소환 딜레이
#endif // #ifdef BSTEST
#define SUMMON_DURATION			(PULSE_REAL_MIN * 20)	// 소환 지속시간
...
#if defined (BSTEST) || defined (LC_RUS)
...
#if defined (TEST_SERVER) || defined (KJTEST) 
#define WAR_CASTLE_SUBNUMBER_MERAC	1	// 공성이 가능한 서브서버 번호
#define RAID_SUBNUMBER		1			// 원정대 레이드 전용 채널 지정
#elif defined (LC_JPN)
#define WAR_CASTLE_SUBNUMBER_MERAC	3	// 공성이 가능한 서브서버 번호
#define RAID_SUBNUMBER		1			// 원정대 레이드 전용 채널 지정
#elif defined(LC_HBK)
#define WAR_CASTLE_SUBNUMBER_MERAC  3   // 공성이 가능한 서브서버 번호
#define RAID_SUBNUMBER		2			// 원정대 레이드 전용 채널 지정
#elif defined(LC_GAMIGO)
#define WAR_CASTLE_SUBNUMBER_MERAC  4   // 공성이 가능한 서브서버 번호
#define RAID_SUBNUMBER		3			// 원정대 레이드 전용 채널 지정
#elif defined(LC_RUS)
#define WAR_CASTLE_SUBNUMBER_MERAC  2   // 공성이 가능한 서브서버 번호
#define RAID_SUBNUMBER		3			// 원정대 레이드 전용 채널 지정
#elif defined(LC_KOR)
#define WAR_CASTLE_SUBNUMBER_MERAC  1   // 공성이 가능한 서브서버 번호
#define RAID_SUBNUMBER		1			// 원정대 레이드 전용 채널 지정
#elif defined(LC_BILA)					// 정식 서비스에서는 뺀다.
#define WAR_CASTLE_SUBNUMBER_MERAC	1	// 공성이 가능한 서브서버 번호
#define RAID_SUBNUMBER		1			// 원정대 레이드 전용 채널 지정
#else
#define WAR_CASTLE_SUBNUMBER_MERAC	1	// 공성이 가능한 서브서버 번호
...
#ifdef LC_JPN
#define REQUITAL_EVENT_ITEM_MAXCOUNT 2
#else
#define REQUITAL_EVENT_ITEM_MAXCOUNT 2
#endif
#endif // REQUITAL_EVENT

#ifdef TRIGGER_SYSTEM	// 트리거 사용
...
// [selo: 101108] 랭킹 시스템
#ifdef SYSTEM_RANKING
#define RANKING_REFRESH_TIME		6		// 갱신하는 시간 (0 ~ 24 사이)

typedef enum _tagRankingType
{
	// 0 ~ 는 JOB 상수로 대처

	RANKING_TYPE_MAXLEVEL = 100,	// 최고 레벨
	RANKING_TYPE_GUARDIAN,			// 최고의 후견인
} RANKING_TYPE;
#endif // SYSTEM_RANKING

#define DUNGEON_DIFFICULTY_NORMAL	0
#define DUNGEON_DIFFICULTY_HARD		1
#define DUNGEON_DIFFICULTY_HELL		2

#ifdef DEV_SYSTEM_GUILD_GRADE_EX
#define GUILD_MAX_GRADE_EX		29
#endif

#ifdef DEV_AUTO_RESET_RAID
...
#ifdef TEST_SERVER
	#define TREASURE_BOX_NPC_REGEN 20*60	// 보물상자 npc regen 시간. 10분  뒤에는 npc가 사라진다.
#else
...
//#ifdef BUGFIX_TITLE_SYSTEM_COUNT
...
#endif // SYSTEM_RANKING_EX

#ifdef POST_SYSTEM
#define NORMAL_TIME		60 * 60 * 4		// 일반 우편 배송 시간 (4시간)
#define PREMIUM_TIME	60 * 60 * 1		// 프리미엄 우편 배송 시간 (1시간)
#define MAX_ATTACHITEM	10				// 첨부 파일 최대 개수 (10개)
#define POSTOFFICE_BACKUP_DAY	1		// 우체국 백업 날짜 (한달에 한번 : 매달 1일 0시)
#define POSTOFFICE_BACKUP_TIME	0		// 우체국 백업 시간 (한달에 한번 : 매달 1일 0시)
#define POSTOFFICE_RETURN_TIME	4		// 우체국 반송 시간 (하루 한번 : 매일 3시)
#define POSTSYSTEM_BACKUP_TIME	60 * 60 * 24 * 10	// 우체국 백업 기준 날짜 (10일)
enum _ePostType
{
	PT_NORMAL =		0,
	PT_PREMIUM =	1,
...
#ifdef KOR_SHUTDOWN
#define DEFAULT_SHUTDOWN_TIME			21600	// 0시~6시까지 shutdown 시간
#define DEFAULT_NOTICE_SHUTDOWN_BEFORE	7200	// 셧다운 2시간전 부터 10분간격으로 공지
#define DEFAULT_SHUTDOWN_EXEC_INTERVAL	600		// 10분마다 미성년자 체크함.
#define DEFAULT_ONE_DAY					86400	// 하루는 86400초
#endif // KOR_SHUTDOWN
...
#if defined (GUILD_MASTER_AUTO_KICK)
...
//#ifdef SYSTEM_EVENT_AUTOMATION_2012
...
#define JEWEL_COMPOSITE_POSITION_ALL		255
//#endif
