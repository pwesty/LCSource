...
// Argument         : unsigned char** pDest
//                  : 복호 데이터 버퍼, new로 할당함, 호출한 곳에서 delete [] 필요
inline int CNM_Decrypt(const unsigned char* pSrc, int nLenSrc, unsigned int nKey, unsigned char pDest[], unsigned char pTmpBuf[])
{
	// 변환 테이블
	#include "CryptNetMsg_TransTable_Reverse"
