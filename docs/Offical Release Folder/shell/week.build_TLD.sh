...
makeBinaries()
{
	...
	do
		...
		make cleanall
		make ${local[i]}
		dirbuf=$rootDir${LOCAL[i]}"/Server/"$folderYear"/"$folderWeek"/"
		mv Connector/Connector GameServer/GameServer_d Helper/Helper SubHelper/SubHelper Messenger/Messenger LoginServer/LoginServer $dirbuf
	done
}
