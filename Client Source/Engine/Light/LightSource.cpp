...
CLightSource::~CLightSource(void)
{
	...
	TR_RemoveTerrainLighting( this );		// yjpark
...
void CLightSource::Read_t( CTStream *pstrm,BOOL bNetwork)        // throw char *
{
	// if receiving over network - skip it
	if (bNetwork) {
		return;
	}
...
void CLightSource::Write_t(CTStream *pstrm,BOOL bNetwork)       // throw char *
{
	// if sending over network - skip it, we have all the info in the savegame
	if (bNetwork) {
		return;
	} 
...
void CLightSource::UpdateTerrains(CPlacement3D plOld, CPlacement3D plNew)
{
	...
	if( ls_ulFlags & LSF_DYNAMIC )			// yjpark |<--
	{
		// if this is point light
		if( !( ls_ulFlags & LSF_DIRECTIONAL ) )
		{
			// add light into terrain for terrain lighting effect
			TR_AddTerrainLighting( this );
		}
		// do not update terrain shadow map
		return;
	}

#ifndef	FINALVERSION			// yjpark |<--
	// if current state is not world editor, do not update terrain shadow map in real-time
	extern BOOL	_bWorldEditorApp;
	extern BOOL	_bInTestGame;
	if( !_bWorldEditorApp || _bInTestGame )
		return ;							// yjpark     -->|

	// loop thru terrains list
	{FORDELETELIST( CTerrain, tr_lnLoadedTerrains, _pGfx->gl_lhLoadedTerrains, ittr) {
		CTerrain *ptrTerrain = ittr;
		ASSERT(ptrTerrain!=NULL);
		// if terrain has been initialized
		if(ptrTerrain->tr_ulTerrainFlags&TR_INITIALIZED) {
			...
			if(ls_ulFlags&LSF_DIRECTIONAL) {
				...
				if(bboxLightPos.HasContactWith(bboxLightOld)) {
					...
					TR_UpdateShadowMap(ptrTerrain, mTerrain, bboxLightAll, TRUE);
				} else {
					// Update part of shadow map where light was before moving
					TR_UpdateShadowMap(ptrTerrain, mTerrain, bboxLightOld, TRUE);
					// Update part of shadow map where light is now
					TR_UpdateShadowMap(ptrTerrain, mTerrain, bboxLightNew, TRUE);
				}
			}
		}
	}}
...
void CLightSource::SetLightSourceWithNoDiscarding( const CLightSource &lsOriginal)
{
	...
	ls_fAmbientAttenuation      = lsOriginal.ls_fAmbientAttenuation;
