...
#define GFX_MAXOCCQUERIES (512L) // maximum allowed occlusion queries

#define GFX_MAXTEXUNITS (8L) // maximum number of supported texture units for multitexturing
#define GFX_MINSTREAMS  (4L) // minimum number of D3D streams in order to support HW T&L (position, normal, texture & color)
#define GFX_MAXLAYERS   (6L) // suggested maximum number of multi-passes per one polygon
#define GFX_MAXLIGHTS   (8L) // maximum number of HW lights allowed
#define GFX_USETEXUNITS (4L) // maximum usable texture units by engine


// common element arrays
extern CStaticStackArray<GFXVertex>    _avtxCommon;
extern CStaticStackArray<GFXNormal>    _anorCommon;
extern CStaticStackArray<GFXColor>     _acolCommon;
extern CStaticStackArray<GFXTexCoord>  _atexCommon[GFX_USETEXUNITS];
extern CStaticStackArray<UWORD>    _auwCommonQuads;
extern CStaticStackArray<UWORD> _auwCommonElements;

// yjpark |<--
// Text element arrays
#define FONT_MAX	14
extern CStaticStackArray<GFXVertex>		_avtxText[FONT_MAX];
extern CStaticStackArray<GFXColor>		_acolText[FONT_MAX];
extern CStaticStackArray<GFXTexCoord>	_atexText[FONT_MAX];
...
extern CStaticStackArray<UWORD>			_auwBtn[MAX_BTN];
// yjpark     -->|
...
// Max shader declaration size
#define MAXSTREAMS (8L)
...
#define GLF_STENCILBUFFER      (1UL<<6)   // has stencil buffer
#define GLF_VERTEXPROGRAM      (1UL<<7)   // supports programable vertex shaders
#define GLF_PIXELPROGRAM       (1UL<<8)   // supports programable pixel shaders
...
#define GLF_D3D_VISIBILITYTEST   (1UL<<15)   // supports visibility test (same as occlusion query!)
...
#define GLF_EXT_OCCLUSIONQUERY      (1UL<<23)   // GL_NV_occlusion_query
...
//안태훈 수정 끝	//(Add Tagent-space Normal Map)(0.1)
	...
	// D3DCAPSINFO
	D3DCAPS8		gl_pd3dCaps;	// Date : 2006-05-16(오후 4:48:07), By eons
	...
	INDEX gl_ctMaxPrimitives;         // maximum number of primitives that can be rendered in one go
	...
	CListHead gl_lhLoadedTerrains;   // list of all terrains 
	BOOL gl_bAllowProbing;

	// for profiling
	INDEX gl_ctWorldElements;      // how many world polygons were rendered in last frame
...
public:
	...
	// swap buffers in a viewport (optionally wait for vertical retrace)
	void SwapBuffers( CViewPort *pvpToSwap, const BOOL bWaitForRetrace=FALSE);
	...
	// check if DualHead or TripleHead is on
	inline BOOL IsCurrentModeDH(void) { return gl_dmCurrentDisplayMode.IsDualHead();   };
	inline BOOL IsCurrentModeTH(void) { return gl_dmCurrentDisplayMode.IsTripleHead(); };
	...
	// is occlusion culling supported
	inline BOOL HasOcclusionCulling(void) {
		if( gl_eCurrentAPI==GAT_OGL) return( gl_ulFlags & GLF_EXT_OCCLUSIONQUERY);
		if( gl_eCurrentAPI==GAT_D3D) return( gl_ulFlags & GLF_D3D_VISIBILITYTEST);
		ASSERT(gl_eCurrentAPI==GAT_NONE);
		return FALSE;
	};

	// get refresh rate (in Hz) of current display mode - may return 0 if unknown!
	inline INDEX GetRefreshHz(void) {
		return 0;
	};
	...
	// has the current driver support for S3TC/DXT1 compression? (not FXT1!)
...
ENGINE_API extern INDEX ClampTextureSize( PIX pixClampSize, PIX pixClampDimension, PIX pixSizeU, PIX pixSizeV);
...
ENGINE_API extern void AddAlphaChannel( UBYTE *pubSrcBitmap, ULONG *pulDstBitmap, PIX pixSize, UBYTE *pubAlphaBitmap=NULL);
// removes 8-bit alpha channel from 32-bit bitmap (in place suported)
ENGINE_API extern void RemoveAlphaChannel( ULONG *pulSrcBitmap, UBYTE *pubDstBitmap, PIX pixSize);

// flips 24 or 32-bit bitmap (iType: 1-horizontal, 2-vertical, 3-diagonal) - in place supported
ENGINE_API extern void FlipBitmap( UBYTE *pubSrc, UBYTE *pubDst, PIX pixWidth, PIX pixHeight, INDEX iFlipType, BOOL bAlphaChannel);
...
// (mip-maps will be filtered with bilinear subsampling)
...
ENGINE_API extern void MakeSubMipmap( ULONG *pulSrc, ULONG *pulDst, PIX pixDstSize);
...
ENGINE_API extern void DitherBitmap( INDEX iDitherType, ULONG *pulSrc, ULONG *pulDst, PIX pixWidth, PIX pixHeight,
																		 PIX pixCanvasWidth=0, PIX pixCanvasHeight=0);
// performs dithering of a 32-bit mipmaps (can be in-place)
ENGINE_API extern void DitherMipmaps( INDEX iDitherType, ULONG *pulSrc, ULONG *pulDst, PIX pixWidth, PIX pixHeight);

// adjust bitmap's saturation and/or hue (hue 0 and saturation 256 are defaults)
ENGINE_API extern void AdjustBitmapColor( ULONG *pulSrc, ULONG *pulDst, PIX pixWidth, PIX pixHeight, 
...
ENGINE_API extern void FilterBitmap( INDEX iFilter, ULONG *pulSrc, ULONG *pulDst, PIX pixWidth, PIX pixHeight,
																		 PIX pixCanvasWidth=0, PIX pixCanvasHeight=0);
...
ENGINE_API extern PIX GetMipmapOffset( INDEX iMipLevel, PIX pixWidth, PIX pixHeight);

// return offset, pointer and dimensions of mipmap of specified size inside texture or shadowmap mipmaps
ENGINE_API extern INDEX GetMipmapOfSize( PIX pixSize, ULONG *&pulFrame, PIX &pixProbeWidth, PIX &pixProbeHeight);
