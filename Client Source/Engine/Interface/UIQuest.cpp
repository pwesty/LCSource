	...
	case MSGLCMD_QUEST_REQ:
		{
			...
			else
			{
				...
				case EVENT_HANAPOS_SK_EVENT:	//[ttos_2009_3_18]: 하나포스 및 SK브로드밴드 가입자 이벤트
					{
						// [100126: selo] 미국은 5채널에서만 버프를 받는다
						// 미국 6번 서버도 버프 가능. [9/2/2010 rumist]
						BOOL bOK = TRUE;
#if defined(G_USA)
						if( _pNetwork->m_iServerCh != 5 && _pNetwork->m_iServerCh != 6 )
						{
							...
							CTString strSysMessage = _S(4812,"초보 지원사의 버프는 5,6채널에서만 받을 수 있습니다.") ; // 번역 
							CUIMsgBox_Info	MsgBoxInfo;
	...
	case MSGLCMD_SYNDICATE_SECESSION_REQ:
		{
			...
			strMessage.PrintF(_S( 6124, "결사대에서 탈퇴 시에는 그 동안 누적되었던 기여도가 대폭 하락하며 아래와 같은 위약금을 지불 하셔야 합니다."));
			MsgBoxInfo.AddString( strMessage );

			strMessage.PrintF(_S( 6125, "위약금:%I64d 나스"), pSyndicateData->GetSecssionNas());	// 위약금은 일단 임의 설정.
			MsgBoxInfo.AddString( strMessage );
...
void CUIQuest::Render()
{
	...
	pDrawPort->PutTextExCX( _S( 587, "퀘스트 목록" ), m_nPosX + QUEST_TAB_CX, m_nPosY + QUEST_TAB_SY,		
		0x6B6B6BFF );
