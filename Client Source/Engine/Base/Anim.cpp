...
// needed to be loaded by an parent object
 ...
 */
//안태훈 수정 시작	//(Defense Code)(2005-01-18)
INDEX CAnimObject::FrameInTime(TIME time) const
{
	ASSERT( ao_AnimData != NULL);
	ASSERT( (ao_iCurrentAnim >= 0) && (ao_iCurrentAnim < ao_AnimData->ad_NumberOfAnims) );
	if(this == NULL) ASSERTALWAYS("Somthnig wrong. this is null?");
	if(ao_AnimData == NULL) return 0;
	if((ao_iCurrentAnim < 0) || (ao_iCurrentAnim >= ao_AnimData->ad_NumberOfAnims)) return 0;
	INDEX iFrameInAnim;

	COneAnim *pCOA = &ao_AnimData->ad_Anims[ao_iCurrentAnim];
	ASSERT(pCOA != NULL);
	ASSERT(ao_AnimData->ad_Anims != NULL);
	if(pCOA == NULL) return 0;
	if(ao_AnimData->ad_Anims == NULL) return 0;
...
//안태훈 수정 끝	//(Defense Code)(2005-01-18)
...
void CAnimObject::GetFrame( INDEX &iFrame0, INDEX &iFrame1, FLOAT &fRatio) const
{
	if(ao_AnimData == NULL || 
		...
		ao_iCurrentAnim >= ao_AnimData->ad_NumberOfAnims ||
