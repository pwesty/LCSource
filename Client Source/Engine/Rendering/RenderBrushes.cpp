...
void CRenderer::PrepareBrush(CEntity *penBrush)
{
	...
	// skip if the brush is already active in rendering
...
//강동민 수정 시작 테스트 클라이언트 작업	06.30
	...
	{
		if( penBrush->en_ulFlags & ENF_EXCEPTREFLECTION)
			return;
	}
	*/
...
{
	const CTranslucentPolygon &tp0 = **(CTranslucentPolygon**)pptp0;
	const CTranslucentPolygon &tp1 = **(CTranslucentPolygon**)pptp1;
	const FLOAT fDelta = tp0.tp_fViewerDistance - tp1.tp_fViewerDistance;
	return (SLONG&)fDelta;
