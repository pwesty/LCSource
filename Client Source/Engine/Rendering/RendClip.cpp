/*
 *  RendClip.cpp - Clipping functions for renderer
...
{
	const INDEX ivx0 = *(const INDEX*)ppvVertex0;
	const INDEX ivx1 = *(const INDEX*)ppvVertex1;
	const FLOAT f0 = *(FLOAT*)(_aVertices+ivx0*sizeof(CViewVertex));
	const FLOAT f1 = *(FLOAT*)(_aVertices+ivx1*sizeof(CViewVertex));
	const FLOAT fDelta = f0 - f1;
	return (SLONG&)fDelta;
	/*   if (f0<f1) return -1;
