...
//static CTextureObject _toInterface;
//static CTextureObject _toInterface2;
//static CTextureObject _toInterface3;
//static CTextureObject _toInterface4;
...
static struct AmmoInfo _aaiAmmo[8] = {
  { NULL/*&_toAShells*/,        &_awiWeapons[4],  &_awiWeapons[5],  0, 0, 0, -9, FALSE }, //  0
  { NULL/*&_toABullets*/,       &_awiWeapons[6],  &_awiWeapons[7],  0, 0, 0, -9, FALSE }, //  1
  { NULL/*&_toARockets*/,       &_awiWeapons[8],  NULL,             0, 0, 0, -9, FALSE }, //  2
  { NULL/*&_toAGrenades*/,      &_awiWeapons[9],  NULL,             0, 0, 0, -9, FALSE }, //  3
  { NULL/*&_toANapalm*/,        &_awiWeapons[11], NULL,             0, 0, 0, -9, FALSE }, //  4
  { NULL/*&_toAElectricity*/,   &_awiWeapons[12], NULL,             0, 0, 0, -9, FALSE }, //  5
  { NULL/*&_toAIronBall*/,      &_awiWeapons[14], NULL,             0, 0, 0, -9, FALSE }, //  6
  { NULL,						NULL,			  NULL,             0, 0, 0, -9, FALSE }, //  7
};
static const INDEX aiAmmoRemap[8] = { 0, 1, 2, 3, 4, 7, 5, 6 };

struct WeaponInfo _awiWeapons[18] = {
  { WEAPON_NONE,            NULL,                 NULL,         FALSE },   //  0
  { WEAPON_KNIFE,           NULL/*&_toWKnife*/,   NULL,         FALSE },   //  1
  { WEAPON_NONE,            NULL,                 NULL,         FALSE },   // 2
  { WEAPON_NONE,            NULL,                 NULL,         FALSE },   // 3
  { WEAPON_NONE,            NULL,                 NULL,         FALSE },   // 4
  { WEAPON_NONE,            NULL,                 NULL,         FALSE },   // 5
  { WEAPON_NONE,            NULL,                 NULL,         FALSE },   // 6
  { WEAPON_NONE,            NULL,                 NULL,         FALSE },   // 7
  { WEAPON_NONE,            NULL,                 NULL,         FALSE },   // 8
  { WEAPON_NONE,            NULL,                 NULL,         FALSE },   // 9  
  { WEAPON_CHAINSAW,        NULL/*&_toWChainsaw*/,NULL,         FALSE },   // 10
  { WEAPON_FLAMER,          NULL/*&_toWFlamer*/,  &_aaiAmmo[4], FALSE },   // 11
  { WEAPON_NONE,            NULL,                 NULL,         FALSE },   // 12
  { WEAPON_NONE,            NULL,                 NULL,         FALSE },   // 13
  { WEAPON_NONE,			NULL,				  NULL, FALSE },		   // 14
...
//0105 9line 삭제. 보더 테두리 나오지 않게 하기.
...
static void HUD_DrawIcon( FLOAT fCenterX, FLOAT fCenterY, CTextureObject &toIcon,
                          COLOR colDefault, FLOAT fNormValue, BOOL bScale)
{
  // determine color
...
*/
  // determine location
  const FLOAT fCenterI = fCenterX*_pixDPWidth  / 1024.0f;
  const FLOAT fCenterJ = fCenterY*_pixDPHeight / (768.0f * _pDP->dp_fWideAdjustment);
  ...
  if(bScale)
  {
	  fHalfSizeI = /*_fResolutionScaling**/1 * ptd->GetPixWidth()  *0.5f;
	  fHalfSizeJ = /*_fResolutionScaling**/1 * ptd->GetPixHeight() *0.5f;
  }
  else{
	  fHalfSizeI = /*_fResolutionScaling**/0.5 * ptd->GetPixWidth()  *0.5f;
	  fHalfSizeJ = /*_fResolutionScaling**/0.5 * ptd->GetPixHeight() *0.5f;
  }
...
static void HUD_DrawSniperMask( void )
{
  ...
  if (_fResolutionScaling>=1.0f)
  {
    ...
    if (_fResolutionScaling<=1.3f) {
      ...
      _pDP->SetTextScaling( 0.4f*_fYResolutionScaling);
      _pDP->SetTextAspect( 1.0f);
      _pDP->SetTextShadow(+2);
      _fIconSize = 19.0f;
      _fLeftX = 162.0f;
      _fLeftYU = 8.0f;
      _fLeftYD = 6.0f;
...
extern void DrawHUD( const CPlayer *penPlayerCurrent, CDrawPort *pdpCurrent, BOOL bSnooping, const CPlayer *penPlayerOwner)
{
  ...
  _tmNow = penPlayerCurrent->en_tmEntityTime;
...
/*  
  ...
  if( fValue > 0.0f) {
    ...
    fCol -= fAdvUnit+fChrUnit*3/2 -fHalfUnit;
    if (fValue<=50.5f) {
      HUD_DrawIcon( fCol+fMoverX, fRow+fMoverY, _toArmorSmall, C_WHITE , fNormValue, FALSE);
    } else if (fValue<=100.5f) {
      HUD_DrawIcon( fCol+fMoverX, fRow+fMoverY, _toArmorMedium, C_WHITE, fNormValue, FALSE);
    } else {
  ...
  if( ptoCurrentAmmo!=NULL && !GetSP()->sp_bInfiniteAmmo) {
    ...
    if( bDrawAmmoIcon) {
      ...
      HUD_DrawIcon( fCol, fRow, *ptoCurrentAmmo, C_WHITE , fNormValue, TRUE);
      fCol -= fAdvUnit+fChrUnit*3/2 -fHalfUnit;
    }
    HUD_DrawText( fCol, fRow, strValue, NONE, fNormValue);
    fCol -= fAdvUnit+fChrUnit*3/2 -fHalfUnit;
    HUD_DrawIcon( fCol+fMoverX, fRow+fMoverY, *ptoCurrentWeapon, C_WHITE , fNormValue, !bDrawAmmoIcon);
...
#define BOMB_FIRE_TIME 1.5f
  ...
  for( i=0; i<MAX_POWERUPS; i++)
  {
    ...
    HUD_DrawIcon(   fCol,         fRow, _atoPowerups[i], C_WHITE , fNormValue, TRUE);
  ...
  if( _penPlayer->m_penMainMusicHolder!=NULL) {
    ...
    if( fNormValue>0) {
      ...
      HUD_DrawIcon(   fCol,      fRow, _toHealth, C_WHITE , fNormValue, FALSE);
    }
  }


  // determine scaling of normal text and play mode
  const FLOAT fTextScale  = (_fResolutionScaling+1) *0.2f;
  ...
  if( !bSinglePlay)
  {
    ...
    { // get player name and mana
      ...
      if( hud_iShowPlayers==1 || hud_iShowPlayers==-1 && !bSinglePlay) {
        ...
        const FLOAT fYOffset = fCharHeight*i + (fOneUnit*2)*_fResolutionScaling;
        if( bCooperative) { 
          _pDP->PutTextR( strName+":", _pixDPWidth-8*fCharWidth, fYOffset, colScore |_ulAlphaHUD);
          _pDP->PutText(  "/",         _pixDPWidth-4*fCharWidth, fYOffset, _colHUD  |_ulAlphaHUD);
          _pDP->PutTextC( strHealth,   _pixDPWidth-6*fCharWidth, fYOffset, colHealth|_ulAlphaHUD);
          _pDP->PutTextC( strArmor,    _pixDPWidth-2*fCharWidth, fYOffset, colArmor |_ulAlphaHUD);
        } else if( bScoreMatch) { 
          _pDP->PutTextR( strName+":", _pixDPWidth-12*fCharWidth, fYOffset, _colHUD |_ulAlphaHUD);
          _pDP->PutText(  "/",         _pixDPWidth- 5*fCharWidth, fYOffset, _colHUD |_ulAlphaHUD);
          _pDP->PutTextC( strScore,    _pixDPWidth- 8*fCharWidth, fYOffset, colScore|_ulAlphaHUD);
          _pDP->PutTextC( strMana,     _pixDPWidth- 2*fCharWidth, fYOffset, colMana |_ulAlphaHUD);
        } else { // fragmatch!
          _pDP->PutTextR( strName+":", _pixDPWidth-8*fCharWidth, fYOffset, _colHUD  |_ulAlphaHUD);
          _pDP->PutText(  "/",         _pixDPWidth-4*fCharWidth, fYOffset, _colHUD  |_ulAlphaHUD);
          _pDP->PutTextC( strFrags,    _pixDPWidth-6*fCharWidth, fYOffset, colFrags |_ulAlphaHUD);
          _pDP->PutTextC( strDeaths,   _pixDPWidth-2*fCharWidth, fYOffset, colDeaths|_ulAlphaHUD);
        }
      }
  ...
  if( bSinglePlay || bCooperative)
  {
    ...
    if( hud_bShowMessages && _penPlayer->m_ctUnreadMessages>0) {
      ...
      TIME tmNow = _penPlayer->en_tmEntityTime + fLerpFactor * _pTimer->TickQuantum;

      FLOAT tmDelta = tmNow-_penPlayer->m_tmAnimateInbox;
      COLOR col = _colHUD;
      ...
      HUD_DrawIcon(   fCol,      fRow, _toMessage, C_WHITE , 0.0f, TRUE);
...
//0102 인터페이스 그리지 않기 위해 HUD_DrawBorder,HUD_DrawText를 지웠다.
//0101 체력 표시  
  // prepare and draw health info
  fValue = ClampDn( _penPlayer->GetHealth(), 0.0f);  // never show negative health
  fNormValue = fValue/TOP_HEALTH;
...
//      HUD_DrawIcon(   fCol,      fRow, _toHealth, C_WHITE , fNormValue, FALSE);
  ...
  //const BOOL bScoreMatch  = !GetSP()->sp_bCooperative && !GetSP()->sp_bUseFrags;
  const BOOL bFragMatch   = !GetSP()->sp_bCooperative &&  GetSP()->sp_bUseFrags;
  COLOR colFrags, colDeaths;//, colHealth, colArmor;
  COLOR colScore  = _colHUD;
  //INDEX iScoreSum = 0;
  ...
  if( !bSinglePlay)
  {
    ...
    FLOAT fCharHeight = (_pfdDisplayFont->GetHeight()-2)*fTextScale;
    // generate and sort by mana list of active players
    BOOL bMaxScore=TRUE, bMaxFrags=TRUE, bMaxDeaths=TRUE;
    hud_iSortPlayers = Clamp( hud_iSortPlayers, -1L, 6L);
    SortKeys eKey = (SortKeys)hud_iSortPlayers;
    if (hud_iSortPlayers==-1) {
           if (bCooperative) eKey = PSK_HEALTH;
...
//    HUD_DrawText(   fCol+fAdv, fRow, strValue,  colMana, 1.0f);
  ...
  */
//0101 싱글플레이어인지 cooperative mode인지...
  /*
  // if single player or cooperative mode
  if( bSinglePlay || bCooperative)
  {
...
/*void HUD_ProcessEffects(HUD_MLData *hud_MLData, CAnyProjection3D& ppr)
{
	...
	hud_MLData->listProcess.clear();

	CItemEffect *pProcItemEffect = &hud_MLData->hud_ItemEffect;
	CItemEffect::ItemEffectGroup *pProcEffectGroup = NULL;

	FLOAT fCurrentTime = _pTimer->GetLerpedCurrentTick();

	pProcEffectGroup = pProcItemEffect->GetPlusEffectGroup();
	if (pProcEffectGroup->m_pEffectGroup != NULL)
	{
		hud_MLData->listProcess.push_back(pProcEffectGroup->m_pEffectGroup);
	}
	
	for (int ep=WEAR_HELMET; ep<WEAR_TOTAL; ++ep)
	{
		pProcEffectGroup = &pProcItemEffect->GetItemEffectGroup()[ep];
	...
	pProcEffectGroup = pProcItemEffect->GetNickEffectGroup();
	if (pProcEffectGroup->m_pEffectGroup != NULL)
	{
		hud_MLData->listProcess.push_back(pProcEffectGroup->m_pEffectGroup);
	}

	if (pProcItemEffect->GetAccessoryEffect() != NULL)
	{
		hud_MLData->listProcess.push_back(pProcItemEffect->GetAccessoryEffect());
	}
	// 여기까지 Process()를 처리할 EffectGroup의 리스트를 구성하였다.

	//Process()를 처리한다.
	for (CEffectGroupManager::my_list::iterator itr=hud_MLData->listProcess.begin(); itr!=hud_MLData->listProcess.end(); ++itr)
	{
		CEffectGroup *pEG = (*itr);
	...
	for (CEffectGroupManager::my_list::iterator delItr=listDelete.begin(); delItr!=listDelete.end(); ++delItr)
	{
		CEffectGroupManager::Instance().Destroy(*delItr);
	}
	...
	inline void operator()(CEffect *pEffect)
	{
		if (pEffect->Playing() && EffectType == pEffect->GetType())
		{
			/*if ((EffectType != ET_MDL) || 
				(EffectType == ET_MDL && bOverDraw && ((CMdlEffect*)(pEffect))->GetOverDraw()) ||
				(EffectType == ET_MDL && !bOverDraw && !((CMdlEffect*)(pEffect))->GetOverDraw()))
			{*/
				pEffect->Render();
			//}
	...
	for(DWORD dwEffectType=0; dwEffectType<(DWORD)ET_COUNT; ++dwEffectType)
	{
		...
		if(effectType != ET_MDL)
		{
			...
			if((*listCreated.begin())->BeginRender(ppr, pdp))
			{
				...
				{
					if ( (*iter)->GetERSubType() == hud_MLData->hud_MI.GetModelInstance()->mi_ERType )
					{
			...
			/*if ( (*listCreated.begin())->GetERSubType() == hud_MLData->hud_MI.GetModelInstance()->mi_ERType )
			{
				if((*listCreated.begin())->BeginRender(ppr, pdp))
				{
		...
		else
		{
			if((*listCreated.begin())->BeginRender(ppr, pdp))
			{
				...
				for(iter=listCreated.begin(); iter!=listCreated.end(); ++iter)
				{
					if ((*iter)->GetERSubType() == hud_MLData->hud_MI.GetModelInstance()->mi_ERType)
					{
						if((*iter)->GetERType() == ER_IN_UI && (*iter)->Playing() && !((CMdlEffect*)(*iter))->GetOverDraw())
							(*iter)->Render();
					}
				}
				(*listCreated.begin())->EndRender(FALSE);
...
void HUD_RenderEffects(HUD_MLData *hud_MLData, CAnyProjection3D& ppr, CDrawPort* pdp)
{
	if (hud_MLData->listProcess.empty())
	{
		return;
	}
	
	CEffectManager::my_list list_MDL;
	CEffectManager::my_list list_Effect;

	for (CEffectGroupManager::my_list::iterator itr=hud_MLData->listProcess.begin(); itr!=hud_MLData->listProcess.end(); ++itr)
	{
		CEffectGroup* pEG = (*itr);

		for (CEffectGroup::vector_effect_key::const_iterator egItr=pEG->GetEffectKeyVector().begin(); 
				egItr!=pEG->GetEffectKeyVector().end(); ++egItr)
		{
			CEffect* pEffect = (*egItr).m_pCreatedEffect;

			if (pEffect->GetType() == ET_SPLINEPATH)
			{
				CSplinePathEffect* spEffect = ((CSplinePathEffect*)pEffect);

				CSplinePathEffect::effect_vector& spEffectList = spEffect->GetvectorEffect();
				CSplinePathEffect::effect_vector::iterator spItr;

				for (spItr = spEffectList.begin(); spItr!=spEffectList.end(); ++spItr)
				{
					CEffect* spUsedEffect = (*spItr);
					if (spUsedEffect->GetType() != ET_MDL)
					{
						list_Effect.push_back(spUsedEffect);
					}
					else
					{
						list_MDL.push_back(spUsedEffect);
					}
				}
			...
			if (pEffect->GetType() != ET_MDL)
			{
				list_Effect.push_back(pEffect);
			}
			else
			{
				list_MDL.push_back(pEffect);
			}
		}
	}

	CEffectManager::my_list::iterator eItr;
	CRenderFunc renderFunc;

	for (int ET_Count = ET_NOTHING; ET_Count< ET_COUNT; ++ET_Count)
	{
		renderFunc.EffectType = (EFFECT_TYPE)ET_Count;
		renderFunc.bOverDraw = FALSE;

		CEffectManager::my_list &listCreated = CEffectManager::Instance().GetCreatedList(ET_Count);
		...
		if((*listCreated.begin())->BeginRender(ppr, pdp))
		{
			if (ET_Count != ET_MDL)
			{
				std::for_each(list_Effect.begin(), list_Effect.end(), renderFunc);
			}
			else
			{
				std::for_each(list_MDL.begin(), list_MDL.end(), renderFunc);
			}
	...
	if (!list_MDL.empty())
	{
		if ((*list_MDL.begin())->BeginRender(ppr, pdp))
		{
			std::for_each(list_MDL.begin(), list_MDL.end(), renderFunc);
			(*list_MDL.begin())->EndRender(FALSE);
		}
	}
}
*/
...
//	_toInterface.SetData_t( CTFILENAME("Textures\\interface_ex.tex"));
//	_toInterface2.SetData_t( CTFILENAME("Textures\\interface_ex2.tex"));
	//_toInterface.SetData_t( CTFILENAME("Data\\interface\\4.tex"));
	//_toInterface2.SetData_t( CTFILENAME("Data\\interface\\1.tex"));
	//_toInterface3.SetData_t( CTFILENAME("Data\\interface\\2.tex"));
	//_toInterface4.SetData_t( CTFILENAME("Data\\interface\\3.tex"));
    ...
    //((CTextureData*)a       .GetData())->Force(TEX_CONSTANT);
	...
	//((CTextureData*)_toInterface.GetData())->Force(TEX_CONSTANT);
	//((CTextureData*)_toInterface2.GetData())->Force(TEX_CONSTANT);
	//((CTextureData*)_toInterface3.GetData())->Force(TEX_CONSTANT);
	//((CTextureData*)_toInterface4.GetData())->Force(TEX_CONSTANT);
