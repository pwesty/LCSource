...
properties:
	...
	214 CEntityPointer	m_penKillEnemy,        //죽어야할 에너미
...
248 BOOL    m_bPolymoph = FALSE,  //폴리모프
...
functions:
	...
	BOOL SetTargetHardForce(CEntity *penPlayer)
	{
		...
		if (!IsValidForEnemy(penPlayer)) 
		{
			if((penPlayer->IsCharacter())) //0426 kwon 우선 몬스터만 공격가능하게...
			{
				// do nothing
				return FALSE;
			}
...
//**********************************************************
	...
	*/
	...
	Die(ECharBaseDeath eEBDeath)
	{
		...
		wait() {
			...
			on (EReconsiderBehavior) : {
				//죽었을때 부활메시지가 오면 다시 액티브로 점프해야 한다.
				m_bIdleAnim = TRUE;
				jump Active();
			}
	...
	Death(EVoid) 
	{
		...
		//1211 //1218 충돌박스를 죽는 애니메이션 끝난 다음에 바꾼다.
...
//강동민 수정 끝 로그인 처리 작업		07.20
	...
	StandardBehavior(EVoid)
	{
		...
		wait()	// 무한 루프
		{
			...
			on (EBegin) : { //!시작 이벤트.
				...
				if (m_penEnemy!=NULL) 
				{
					call Active();  //타켓이 있다면 active
				} 
				else 
				{
					call Inactive(); //타켓이 없다면 inactive
				}

			};
			// if received damage
			on (ECharBaseDamage eEBDamage) : { //데미지를 받았다면,
				// if should already blow up
				if (ShouldBlowUp()) { //폭발해야 한다면,
					// blow up now
					EBlowUp eBlow;
					eBlow.vDamage = m_vDamage;
					eBlow.vTranslation = en_vCurrentTranslationAbsolute;
	...
	// WSS_DRATAN_SEIGEWARFARE 2007/08/01 교감 애니메이션
