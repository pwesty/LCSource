...
void CTextureControl::AddControl(CTString strLabelText, INDEX *piItemID)
{
  ...
  if(txc_Label.Create("texture",LABEL_STYLES,RECT_LEFT_SIDE,&DLGSHADER,iCtrlID))
  {
    txc_Label.SetFont(DLGSHADER.GetFont(),FALSE);
    txc_Label.SetWindowText((const char*)(strLabelText+": "));
  }
...
void CTexCoordControl::AddControl(CTString strLabelText, INDEX *piItemID)
{
  ...
  if(txcc_Label.Create("uvmap",LABEL_STYLES,RECT_LEFT_SIDE,&DLGSHADER,iCtrlID))
  {
    txcc_Label.SetFont(DLGSHADER.GetFont(),FALSE);
    txcc_Label.SetWindowText((const char*)(strLabelText+": "));
  }
...
void CColorControl::AddControl(CTString strLabelText, COLOR *pcolColor)
{
  ...
  if(cc_Label.Create("color",LABEL_STYLES,RECT_LEFT_SIDE,&DLGSHADER,iCtrlID))
  {
    cc_Label.SetFont(DLGSHADER.GetFont(),FALSE);
    cc_Label.SetWindowText((const char*)(strLabelText+": "));
  }
  
  if(cc_Button.Create("",WS_CHILD|WS_VISIBLE|BS_OWNERDRAW,RECT_RIGHT_SIDE,&DLGSHADER,iCtrlID+1))
  {
...
void CFloatControl::AddControl(CTString strLabelText, FLOAT *pFloat)
{
  ...
  if(fc_Label.Create("float",LABEL_STYLES,RECT_LEFT_SIDE,&DLGSHADER,iCtrlID))
  {
...
void CFlagControl::AddControl(CTString strLabelText, INDEX iFlagIndex, ULONG ulFlags)
{
  ...
  if(fc_CheckBox.Create((const char*)strLabelText,CHECK_STYLES,RECT_FULL_SIZE,&DLGSHADER,iCtrlID+1))
  {
...
BEGIN_MESSAGE_MAP(CDlgClient, CDialog)
	...
	ON_LBN_SELCHANGE(IDC_ANIMEFFECT_FLAGS, OnLbAnimEffectFlags)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
//안태훈 수정 시작	//(Effect Add & Modify for Close Beta)(0.1)
void CDlgClient::OnLbAnimEffectFlags() 
{
	CListBox &lb = *(CListBox*)GetDlgItem(IDC_ANIMEFFECT_FLAGS);
	...
	{
		if(lb.GetSel(i) > 0) flags |= 1UL << i;
	}
	ae.GetReservedEG((INDEX)niSelected.ni_pPtr)->SetFlags(flags);
...
void CDlgClient::OnSelendokCbParentbone() 
{
  ...
  INDEX iNewBoneParent = ska_GetIDFromStringTable(strParentName);
...
void CDlgClient::OnSelendokCbParentmodel() 
{
  ...
  // if pmiSelected wasnt added as include of parent
...
void CDlgClient::OnSelendokCbShader() 
{
  ...
  CTString strNewShaderName = (CTString)"Shaders\\" + strName + ".sha";
...
void CDlgClient::OnShowWindow(BOOL bShow, UINT nStatus) 
{
  ...
  if((bShow==TRUE) && (cbShader!=NULL))
  {
    ...
    for(INDEX ifn=0; ifn<afnShaders.Count(); ifn++)
    {
      ...
      cbShader->AddString((const char*)fnShader.FileName());
    }
  }
...
void CDlgClient::OnBtBrowseTexture() 
{
  ...
  if(ni.ni_iType == NT_TEXINSTANCE) {
    ...
    CTFileName fnTexture = _EngineGUI.FileRequester( "Open texture files", 
      FILTER_TEXTURE, "Open directory", "Data\\Models\\", "", NULL);
    if(fnTexture=="") {
      return;
    }
...
void CDlgClient::OnBtCalcAllframesBbox() 
{
  ...
  theApp.UpdateAllFramesBBox(*pmi);
  /*
  FLOATaabbox3D bbox;
