...
static UINT _uiCacheSize = 0;
static UINT _uiMinStripSize = 0;
static BOOL _bOnlyOptimizeLists = FALSE;
static BOOL _bStichStrips = TRUE;
static BOOL _bRemapVertices = TRUE;
static CTString strSurfaceName;

#define CACHESIZE_GEFORCE1_2 16 // GeForce1 and 2 cache size
#define CACHESIZE_GEFORCE3   24 // GeForce3 cache size
...
{
  _uiCacheSize = CACHESIZE_GEFORCE3;
  _uiMinStripSize = 0;
  _bOnlyOptimizeLists = TRUE;
  _bStichStrips = FALSE;
  _bRemapVertices = TRUE;


  CStaticArray<INDEX> aiRemapTable; 
  CStaticArray<UWORD> auwTriLists;

  // create vertex remap table
  INDEX ctVertices = mlod.mlod_aVertices.Count();
  aiRemapTable.New(ctVertices);
#if _DEBUG
  memset(&aiRemapTable[0],0xCD,sizeof(INDEX) * ctVertices);
#endif
  ...
  for(INDEX imsrf=0;imsrf<ctmsrf;imsrf++) {
    ...
    PrimitiveGroup *papgIndices = NULL;
    PrimitiveGroup *papgRemapedIndices = NULL;
    PrimitiveGroup *papgFinalIndices = NULL;
    UWORD ctGroups;

    MeshSurface &msrf = mlod.mlod_aSurfaces[imsrf];
    INDEX ctind = msrf.msrf_auwTriangles.Count()*3;
    strSurfaceName = CTString(0,"%s(%s)",(const char*)mlod.mlod_fnSourceFile,(const char*)ska_IDToString(msrf.msrf_iSurfaceID));

    // generate new indices for surface 
    GenerateStrips(&msrf.msrf_auwTriangles[0].mt_uwVertexIndex[0],ctind,&papgIndices,&ctGroups);

    INDEX ctwm = mlod.mlod_aWeightMaps.Count();
    INDEX ctmm = mlod.mlod_aMorphMaps.Count();
    // if remap indices flag is on
    if(_bRemapVertices) {
      // remap vertex order in surface
      RemapIndices(papgIndices,ctGroups,ctVertices,&papgRemapedIndices);
      papgFinalIndices = papgRemapedIndices;
    } else {
      papgFinalIndices = papgIndices;
    }

    ctind = papgFinalIndices->numIndices;
    UWORD *puwOriginalIndices = &msrf.msrf_auwTriangles[0].mt_uwVertexIndex[0];
    // fill remap table
    for(INDEX iind=0;iind<ctind;iind++) {
      INDEX iVtxOrg = puwOriginalIndices[iind];
      INDEX iVtxOld = papgIndices->indices[iind];
      INDEX iVtxNew = papgFinalIndices->indices[iind];
      aiRemapTable[iVtxOld + msrf.msrf_iFirstVertex] = iVtxNew + msrf.msrf_iFirstVertex;
    }

    msrf.msrf_auwTriangles.Clear();
    msrf.msrf_auwTriangles.New(papgFinalIndices->numIndices/3);
    memcpy(&msrf.msrf_auwTriangles[0],&papgFinalIndices->indices[0],sizeof(UWORD)*papgFinalIndices->numIndices);
#if 0
    // dump triangles
    const INDEX ctst = msrf.msrf_auwTriangles.Count();
    CPrintF("Surface %s\n",ska_IDToString(msrf.msrf_iSurfaceID));
    for(INDEX ist=0;ist<ctst;ist++) {
      MeshTriangle &st = msrf.msrf_auwTriangles[ist];
      CPrintF("%d,%d,%d\n",st.mt_uwVertexIndex[0],st.mt_uwVertexIndex[1],st.mt_uwVertexIndex[2]);
    }
#endif
    ...
    if(papgRemapedIndices!=NULL) {
      delete[] papgRemapedIndices;
      papgRemapedIndices = NULL;
    }
  ...
  for(INDEX ivx=0;ivx<ctVertices;ivx++) {
    INDEX iIndex = aiRemapTable[ivx];
    ASSERT(iIndex!=0xCDCDCDCD);
  }
#endif
  // remap indices in mesh lod
  // RemapMeshIndices(mlod,&aiRemapTable[0],aiRemapTable.Count());
  // if flag to remap vertices is set
  if(_bRemapVertices) {
    // remap vertices in mesh lod
    RemapMeshVertices(mlod,&aiRemapTable[0],aiRemapTable.Count());
  }
}
					...
					PrimitiveGroup** primGroups, unsigned short* numGroups)
{
	...
	stripifier.Stripify(tempIndices, _uiCacheSize, _uiMinStripSize, maxIndex, tempStrips, tempFaces);

	//stitch strips together
	IntVec stripIndices;
	unsigned int numSeparateStrips = 0;

	if(_bOnlyOptimizeLists)
	{
		int i = 0;
		int size = 0;
		...
		*numGroups = 1;
		...
		for(i = 0; i < size; i++)
		{
			int facesSize = tempStrips[i]->m_faces.size();
	...
	{
		stripifier.CreateStrips(tempStrips, stripIndices, _bStichStrips, numSeparateStrips);

		//if we're stitching strips together, we better get back only one strip from CreateStrips()
		ASSERT( (_bStichStrips && (numSeparateStrips == 1)) || !_bStichStrips);
									 ...
									 const unsigned short maxIndex)
{
	...
	{	
		...
		else 
		{
			...
			{
				// CPrintF("%s - BuildStripifyInfo: > 2 triangles on an edge... uncertain consequences\n",(const char*)strSurfaceName);
			}
			else
			{
...
inline int NvStripifier::GetNextIndex(const WordVec &indices, NvFaceInfo *face){
	...
	if (fv0 != v0 && fv0 != v1){
		if ((fv1 != v0 && fv1 != v1) || (fv2 != v0 && fv2 != v1)){
			CPrintF("%s - GetNextIndex: Triangle doesn't have all of its vertices\n",(const char*)strSurfaceName);
			CPrintF("%s - GetNextIndex: Duplicate triangle probably got us derailed\n",(const char*)strSurfaceName);
		}
		return fv0;
	}
	if (fv1 != v0 && fv1 != v1){
							...
							NvStripInfoVec &outStrips, NvFaceInfoVec& outFaceList)
{
	...
	//clean up
	int i = 0;
	int size = allStrips.size();
	for(i = 0; i < size; i++)
	{
		delete allStrips[i];
	}
	
	size = allEdgeInfos.size();
	for (i = 0; i < size; i++)
	{
