/*
 *  Serious Sam
 *  Copyright (c) 2000, CroTeam. All rights reserved.
 *
 */
...
static BOOL _bIsTcpServer = FALSE;
...
//0105  
  //_pNetwork->IsTcpServer =FALSE;//TRUE;//			
  _pNetwork->IsTcpServer = _bIsTcpServer;		// by seo-40225
...
BOOL Init(int argc, char* argv[])
{
  ...
  // TCP/IP stream 방식으로 띄위기 위해서 첫번째 파라미터 수정 by seo-40225
  if (argv[1] && !stricmp(argv[1], "defaultcoop_tcp")) {
	argv[1][strlen("defaultcoop")] = '\0';
	_bIsTcpServer = TRUE;  
  }
...
//0126  
//  ded_strLevel = "Levels\\LevelsMP\\3_3_CorridorOfDeath.wld";
  ded_strLevel = "Levels\\LevelsMP\\final.wld"; //new.wld";
...
int SubMain(int argc, char* argv[])
{
  ...
  // start first round
  RoundBegin();
  _pNetwork->ga_srvServer.srv_assoSessions[0].sso_bSendStream = TRUE;
