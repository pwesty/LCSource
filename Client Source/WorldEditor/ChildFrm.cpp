...
extern INDEX wed_bSaveTestGameFirstTime = FALSE;
ENGINE_API extern INDEX snd_iFormat;

// 테스트 게임이 실행되는 것을 제한합니다.
//#define RESTRICT_TEST_GAME
...
BEGIN_MESSAGE_MAP(CChildFrame, CMDIChildWnd)
	...
	ON_COMMAND(ID_POLYGON_ATTR, OnPolygonAttr)
...
void CChildFrame::RememberChildConfiguration( INDEX iViewConfiguration)
{
  ...
  // here we will recieve splitter's sizes
...
void CChildFrame::SetChildConfiguration( INDEX iViewConfiguration)
{
  ...
  for( j=0; j<CC.m_iVerticalSplitters; j++)
  {
    for( i=0; i<CC.m_iHorizontalSplitters; i++)
    {
	  ...
	  pWEDView->SetViewIndex(j*2+i, iViewConfiguration);
      pWEDView->m_vpViewPrefs = CC.m_vpViewPrefs[ j*2 + i];
	  pWEDView->m_vpViewPrefs.m_PaperColor = theApp.m_vpPrevViewPrefs[iViewConfiguration][j*2+i].m_PaperColor;
...
void CChildFrame::TestGame( BOOL bFullScreen) 
{
  ...
  //if(0/* (!pDoc->m_bWasEverSaved && wed_bSaveTestGameFirstTime) || pDoc->IsModified()*/)
    // [070607: Su-won] 테스트게임실행시 저장하지 않고 실행.
  // 콘솔창에서 wed_bSaveTestGameFirstTime의 값을 변경할 수 있음.
  if( wed_bSaveTestGameFirstTime )
  ...
  { // use the saved world
    fnmWorldToPlay = CTString(pDoc->GetPathName());
    try {
  ...
  if( bFullScreen) 
  {
    ...
    wcFullScreen.lpszMenuName = APPLICATION_NAME;
    wcFullScreen.lpszClassName = APPLICATION_NAME;
    ...
    hWndFullScreen = CreateWindowEx(
      ...
      //WS_POPUP,
      // [070607: Su-won] 테스트게임 풀스크린모드로 실행시 윈도우모드로 실행
      // 풀스크린으로 할 경우 게임화면이 없어지지 않는 경우가 발생함.
    ...
    ::SetWindowText( hWndFullScreen, achWindowTitle);
    ::ShowWindow(    hWndFullScreen, SW_SHOWNORMAL);

    // set Direct3D fullscreen (after window)
    if( gat==GAT_D3D) {
    	// [070607: Su-won] 테스트게임 풀스크린모드로 실행시 윈도우모드로 실행
    	// 풀스크린으로 할 경우 게임화면이 없어지지 않는 경우가 발생함.
        //const BOOL bRes = _pGfx->SetDisplayMode( gat, 0, pixSizeI, pixSizeJ, dd);
		const BOOL bRes = _pGfx->SetDisplayMode( gat, 0, 0, 0, dd);
      if( !bRes) {
        WarningMessage( "Unable to setup full screen display.");
...
//안태훈 수정 시작	//(Add Sun Moon Entity and etc)(0.2)
  g_fGWTime = 0.0f;							// yjpark |<--
//안태훈 수정 끝	//(Add Sun Moon Entity and etc)(0.2)
  CWorld*	pwld = &(pDoc->m_woWorld);
...
void CChildFrame::OnSceneRenderingTime() 
{
	...
	if(m_bSceneRenderingTime)
	{
		extern ENGINE_API INDEX _iStatsMode;
		_iStatsMode = 3; // send it to engine
	}
...
void CChildFrame::OnPolygonAttr() 
{
	// TODO: Add your command handler code here
	extern BOOL	_bWorldEditorApp;
	extern BOOL _bShowPolygonAttribute;
	if( _bWorldEditorApp )
		_bShowPolygonAttribute = !_bShowPolygonAttribute;
}
...
void CChildFrame::OnClose() 
{
	...
	if(pMainFrame)
	{
		if(pMainFrame->m_pNpcManagerDialog)
		{
			pMainFrame->m_pNpcManagerDialog->DestroyWindow();
		}
		if(pMainFrame->m_pMobDataDialog)
		{
			pMainFrame->m_pMobDataDialog->DestroyWindow();
		}
	}	
