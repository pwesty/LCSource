...
void CBrowser::AddDirectoryRecursiv(CVirtualTreeNode *pOneDirectory, HTREEITEM hParent)
{
  ...
  m_TreeCtrl.SetItemText( InsertedDir, pOneDirectory->vtn_strName);
  m_TreeCtrl.SetItemImage( InsertedDir, pOneDirectory->vtn_itIconType,
                           pOneDirectory->vtn_itIconType + NO_OF_ICONS);

  // Now add this directory's subdirectories recurively
...
void CBrowser::OnCreateDirectory() 
{
  ...
  if( dlg.DoModal() == IDOK)
  {
    ...
    if( m_TreeCtrl.GetCount() == 0)
    {
      ...
      m_VirtualTree.vtn_strName = CTString( dlg.m_strDirectoryName);
    ...
    else
    {
      ...
      pvtnNewDir->vtn_strName = CTString( dlg.m_strDirectoryName);
...
void CBrowser::OnDeleteDirectory() 
{
  ...
  if( ::MessageBox( this->m_hWnd, "Do You realy want to delete directory and all its subdirectories?",
...
INDEX CBrowser::GetSelectedDirectory( CTString strArray[])
{
  ...
  FOREVER
  {
    CTString strItemName = CTString( m_TreeCtrl.GetItemText(pItem));
...
HTREEITEM CBrowser::GetVirtualDirectoryItem( CTString strArray[], INDEX iSubDirsCt)
{
  ...
  for( j=iSubDirsCt-1; j>=0; j--)
  {
    ...
    FOREVER
    {
      ...
      CTString strItemName = CTString( m_TreeCtrl.GetItemText(pItem));
      // is this subdirectory's name same as one in list of selecteds along given path
      if( strArray[ j] == strItemName)
      {
        // subdirectory found, if it has childs or selected subdirectories counter reached 0
        ...
        // we have no more childs and counter is not 0 which means
        else
        {
          // that we didn't successed
          bSucess = FALSE;
          break;
        }
      }
      ...
      else
      {
        ...
        // if we have wrapped back to start of items list, we didn't found any dir with
        // selected name
        if( pStartItem == pItem)
        {
          // so we wasn't successefull
          bSucess = FALSE;
          break;
        }
      }
...
CTFileName CBrowser::GetIOFileName(CTString strTitle, BOOL bSave) 
{
  ...
  OPENFILENAME ofnSaveVirtualTree;
  ...
  if( bSave)
  {
    bResult=GetSaveFileName( &ofnSaveVirtualTree);
  }
  else
  {
    bResult=GetOpenFileName( &ofnSaveVirtualTree);
  }
  if( !bResult)
  {
    m_TreeCtrl.SetFocus();
...
void CBrowser::OnLoadVirtualTree() 
{
  ...
  {
    if( ::MessageBox( this->m_hWnd, "Current virtual directory not saved. Do You want to continue anyway?",
                    "Warning !", MB_YESNO | MB_ICONWARNING | MB_DEFBUTTON1 | 
                    MB_TASKMODAL | MB_TOPMOST) != IDYES)
    {
      m_TreeCtrl.SetFocus();
...
void CBrowser::OnRenameDirectory() 
{
  if( m_TreeCtrl.GetCount() != 0)
  {
    ...
    if( dlg.DoModal() == IDOK)
    {
      ...
      pVTN->vtn_strName = CTString( dlg.m_strDirectoryName);
      m_TreeCtrl.SetItemImage( pSelectedItem, pVTN->vtn_itIconType,
                                              pVTN->vtn_itIconType + NO_OF_ICONS);
      m_TreeCtrl.SetItemText( pSelectedItem, pVTN->vtn_strName);
      m_bVirtualTreeChanged = TRUE;
    }
    m_TreeCtrl.SetFocus();
  }
