...
//강동민 수정 시작 물 퍼포먼스 작업
	  ...
	  SET_TRI_STATE_TO_CTRL( m_bShootThru,    BPOF_WATER);
//강동민 수정 끝 물 퍼포먼스 작업
...
//강동민 수정 시작 물 퍼포먼스 작업
	  ...
	  TRI_STATE_CTRL_TO_FLAGS( m_bShootThru, BPOF_WATER, FALSE, FALSE);
//강동민 수정 끝 물 퍼포먼스 작업
...
void CDlgPgPolygon::InitComboBoxes(void)
{
  ...
  for(INDEX iFriction=0; iFriction<MAX_UBYTE; iFriction++)
  {
    ...
    INDEX iAddedAs = m_ComboFriction.AddString( strFrictionName);
  }
  ...
  if( pDoc->m_selPolygonSelection.Count() != 0)
  {
    ...
    if( bEnableMirror)
    {
      ...
      for(INDEX iMirror=1; iMirror<MAX_UBYTE; iMirror++)
      {
        ...
        m_ComboMirror.AddString( strMirrorName);
      }
    }
  }
}
