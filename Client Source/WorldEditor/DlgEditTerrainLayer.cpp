...
void CDlgEditTerrainLayer::DoDataExchange(CDataExchange* pDX)
{
  ...
  DDX_Control(pDX, IDC_TRL_MASK_STRETCH, m_ctrlMaskStretch);
  ...
  if( pDX->m_bSaveAndValidate != FALSE)
  {
    ...
    if(ptlLayer!=NULL)
    {
      ...
      ptlLayer->tl_strName=m_strLayerName;
	  ...
	  SLONG	slOldMaskStretch = ptlLayer->tl_slMaskStretch;
	  ptlLayer->tl_slMaskStretch = m_ctrlMaskStretch.GetCurSel();			// yjpark

      TRL_ApplyLayerMatrix(GetTerrain(), GetLayerIndex());
      TR_UpdateTopMap(GetTerrain());

	  if( slOldMaskStretch != ptlLayer->tl_slMaskStretch )					// yjpark |<--
	  {
	    CTerrain	*ptrTerrain = GetTerrain();
	    CTerrainImp	*ptr = ptrTerrain->tr_ptrTerrain;
		PIX			pixOldWidth = ( ptr->tr_pixHeightMapWidth - 1 ) << slOldMaskStretch;
		PIX			pixOldHeight = ( ptr->tr_pixHeightMapWidth - 1 ) << slOldMaskStretch;
		PIX			pixNewWidth = ( ptr->tr_pixHeightMapWidth - 1 ) << ptlLayer->tl_slMaskStretch;
		PIX			pixNewHeight = ( ptr->tr_pixHeightMapWidth - 1 ) << ptlLayer->tl_slMaskStretch;

	    TRL_UpdateLayerSize( ptrTerrain, GetLayerIndex(), pixNewWidth, pixNewHeight, pixOldWidth, pixOldHeight );
	  }																		// yjpark     -->|
    }
  }
}
...
// yjpark |<--
BOOL CDlgEditTerrainLayer::OnInitDialog() 
{
	...
	CTerrainImp		*ptr = ptrTerrain->tr_ptrTerrain;
	CTerrainLayer	*ptlLayer=GetLayer();
	UWORD			uwMaskWidth = ptr->tr_pixHeightMapWidth - 1;
	UWORD			uwMaskHeight = ptr->tr_pixHeightMapHeight - 1;
	UWORD			uwMaskLarge = uwMaskWidth > uwMaskHeight ? uwMaskWidth : uwMaskHeight;
	SLONG			slMaskStretch = 0;
	CString			strTemp;

	if( ptlLayer != NULL )
	{
		while( ( uwMaskLarge << slMaskStretch ) <= 2048 )
		{
			strTemp.Format( _T("%d x"), 1 << slMaskStretch );
			m_ctrlMaskStretch.AddString( strTemp );
			slMaskStretch++;
		}

		m_ctrlMaskStretch.SetCurSel( ptlLayer->tl_slMaskStretch );
	}
