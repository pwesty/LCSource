	...
	: CDialog(CDlgLightAnimationEditor::IDD, pParent)
{
  ...
  // try to load animaiton that was last editted
  try
  {
    CTFileName fnLastEditted = CTString( theApp.GetProfileString("World editor", "Last editted light animation", DEFAULT_ANIMATION_FILE));
...
{
  theApp.WriteProfileString("World editor", "Last editted light animation", m_padAnimData->GetName());
...
void CDlgLightAnimationEditor::InitializeData(void)
{
  ...
  SetWindowText( "Editting animation: " + strTitle);
...
void CDlgLightAnimationEditor::StoreData(void)
{
  ...
  m_padAnimData->SetName( iLightAnimation, CTString(m_strLightAnimationName));
  //------------ Prepare new array of frames for current animation (key frames changing is
  // not applyed here but in control LMB down handler)
...
void CDlgLightAnimationEditor::SpreadFrames(void)
{
  ...
  do
  {
    ...
    for (iKeySearcher=iStart+1; iKeySearcher<ctFrames; iKeySearcher++)
    {
...
void CDlgLightAnimationEditor::InitLightAnimationCombo(void)
{
  ...
  for(INDEX iAnimation=0;iAnimation<m_padAnimData->GetAnimsCt(); iAnimation++)
  {
    ...
    // add animation name
    m_LightAnimationCombo.AddString( aiInfo.ai_AnimName);
  }
}
