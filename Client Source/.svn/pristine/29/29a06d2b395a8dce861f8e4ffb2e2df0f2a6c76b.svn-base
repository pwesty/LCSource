...
void CDlgCreateAnimatedTexture::InitAnimationsCombo(void)
{
  ...
  if( m_ptdCreated != NULL)
  {
    ...
    for( INDEX iAnim=0; iAnim<m_ptdCreated->GetAnimsCt(); iAnim++)
    {
      ...
      m_ctrlAnimationsCombo.AddString(aiInfo.ai_AnimName);
    }
  }
  else
  {
...
void CDlgCreateAnimatedTexture::RefreshTexture(void)
{
  ...
  try
  {
    ...
    char *pScript = (char *) AllocMemory( strlen(m_strEditScript)+1);
    strcpy( pScript, m_strEditScript);
    fileScript.WriteRawChunk_t( pScript, strlen(m_strEditScript)+1);
    fileScript.Close();
    FreeMemory( pScript);
...
void CDlgCreateAnimatedTexture::OnCreateTexture() 
{
  ...
  {
    fnSaveName = CTString( m_strCreatedTextureName);
  }
  ...
  CopyFile( fnFullTempScript, fnFullFinalScript, FALSE);
  CopyFile( fnFullTempTexture, fnFullFinalTexture, FALSE);
