...
void CDlgTEOperationSettings::DoDataExchange(CDataExchange* pDX)
{
  ...
  if(pDX->m_bSaveAndValidate == FALSE)
  {
    ...
    if(ptrTerrain!=NULL)
    {
      GetDlgItem(IDC_EQUALIZE_VALUE)->EnableWindow( TRUE);
  	  m_fClampAltitude=FLOAT(theApp.m_uwEditAltitude)/65535*TR_GetTerrainSize(ptrTerrain)(2);
    }
    else
    {
  ...
  if(pDX->m_bSaveAndValidate != FALSE)
  {
    ...
    if(ptrTerrain!=NULL)
    {
	    theApp.m_uwEditAltitude=m_fClampAltitude/TR_GetTerrainSize(ptrTerrain)(2)*65535;
    }
