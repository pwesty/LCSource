...
#define INI_PRIMITIVE_READ( strname, default_val)                               \
	strcpy( strIni, theApp.GetProfileString( "World editor prefs", strPrimitiveType+" "+strname, default_val))
#define INI_PRIMITIVE_WRITE( strname)                                   \
	theApp.WriteProfileString( "World editor prefs", strPrimitiveType+" "+strname, strIni)

#define INI_READ( strname, default_val)                               \
	strcpy( strIni, theApp.GetProfileString( "World editor prefs", strname, default_val))
#define GET_FLAG( var)                                        \
...
//강동민 수정 시작 2차 작업			05.19
// 아래 SET_FLAG( var)을 SET_FLAG2( var)로 수정함.
// MYSQL과의 충돌을 피하기 위해서...
//강동민 수정 끝 2차 작업			05.19
#define SET_FLAG2( var)                                        \
...
#define INI_WRITE( strname)                                   \
	theApp.WriteProfileString( "World editor prefs", strname, strIni)

void InitializeGame(void)
{
	try {
#ifndef	_DEBUG
#	define GAMEDLL "GameGUI"+_strModExt+".dll"
#else
#	define GAMEDLL "GameGUI"+_strModExt+"D.dll"
#endif 

		CTFileName fnmExpanded;

		fnmExpanded = _fnmApplicationPath + _fnmApplicationExe.FileDir() + CTString(GAMEDLL);
//강동민 수정 시작 테스트 클라이언트 작업	06.30
		//OutputDebugString(fnmExpanded);
...
{
	// register message
	_uiMessengerMsg = RegisterWindowMessage("Croteam Messenger: Incoming Message");
	_uiMessengerForcePopup = RegisterWindowMessage("Croteam Messenger: Force Popup Message");
...
//#endif
	...
	m_fnDistributionNoiseTexture=CTFILENAME("Data\\Textures\\Editor\\RandomNoise.tex");
	m_fnContinousNoiseTexture=CTFILENAME("Data\\Textures\\Editor\\RandomNoise.tex");
...
CWorldEditorApp::~CWorldEditorApp()
{
	//Su-won
	delete _pGameState;
	// [2012/02/03 : Sora] UIManager바뀜
	//delete _pUIMgr;
	if(CUIManager::isCreated())
	{
		CUIManager::getSingleton()->DestroyAll();
		CUIManager::getSingleton()->destroy();
	}
}

CTFileName GetSourceSafePath(void)
{
	// create the registry key
	HKEY hkey;
	LONG lRes = RegOpenKeyEx(HKEY_LOCAL_MACHINE, "SOFTWARE\\Microsoft\\SourceSafe", 0, KEY_ALL_ACCESS, &hkey);
	// if not succeseful
	...
	LONG lResult = RegQueryValueEx(hkey, "SCCServerPath", 0, &ctType, (unsigned char *)achrKeyValue, &ctLength);
	if( lResult != ERROR_SUCCESS)
	{
		//WarningMessage("QueryRegistryValueEx() returned: %s", GetWindowsError(lResult));
		return CTString("");
	}

	CTFileName fnResult = CTString( achrKeyValue);
	fnResult.PrintF("%sss.exe", (const char *) fnResult.FileDir() );
	return fnResult;
}
...
static CTString cmd_strConvertList;
...
void CWorldEditorApp::MyParseCommandLine(void)
{
	...
	FOREVER {
		...
		if (strWord=="") {
			cmd_strOutput+="\n";
			m_lpCmdLine = (char *)(const char*)_strCmd;
			return;
		...
		} else if (strWord=="+convert") {
			cmd_strConvertList = GetNextParam();
		} else {
			m_lpCmdLine = (char *)(const char*)_strCmd;
			return;
		}
...
BOOL CWorldEditorApp::SubInitInstance()
{
	...
	// obtain source safe path
	m_fnSSPath = GetSourceSafePath();

	// set flag that this is world editor
	...
	if (_strModExt=="") {
		...
		SetRegistryKey( "LastChaos\\"+_strModExt);
	}

	CPrintF("%s", cmd_strOutput);

	// if the registry is not set yet
	if (GetProfileString( "World editor prefs", "Default primitive texture", "")=="") {
...
/*    _spawnlp(_P_WAIT, "regedit.exe", 
	...
	_pShell->DeclareSymbol("user void WED_DeselectAll(void);", &WED_DeselectAll);
	...
	m_gatFullScreen = GAT_D3D;
	_EngineGUI.SetFullScreenModeToRegistry(   "Display modes", m_dmFullScreen, GAT_D3D);

	m_iApi=GAT_D3D;
	m_iApi=AfxGetApp()->GetProfileInt("Display modes", "SED Gfx API", GAT_D3D);
...
//	pUIManager->Create();
	...
	try
	{
		...
		// load error texture
		DECLARE_CTFILENAME( fnViewIcons, "Textures\\Editor\\Error.tex");
	...
	try
	{
		...
		DECLARE_CTFILENAME( fnEntityMarker, "Data\\Models\\Editor\\EntityMarker.mdl");
		...
		DECLARE_CTFILENAME( fnEntityMarkerTex, "Data\\Models\\Editor\\EntityMarker.tex");
		...
		DECLARE_CTFILENAME( fnPortalMarker, "Data\\Models\\Editor\\PortalMarker.mdl");
		...
		DECLARE_CTFILENAME( fnPortalMarkerTex, "Data\\Models\\Editor\\PortalMarker.tex");
		...
		DECLARE_CTFILENAME( fnEmptyBrush, "Data\\Models\\Editor\\EmptyBrush.mdl");
		...
		DECLARE_CTFILENAME( fnEmptyBrushTex, "Data\\Models\\Editor\\EmptyBrush.tex");
		...
		// load range sphere
		DECLARE_CTFILENAME( fnRangeSphere, "Data\\Models\\Editor\\RangeSphere.mdl");
		...
		DECLARE_CTFILENAME( fnRangeSphereTex, "Data\\Models\\Editor\\RangeSphere.tex");
		...
		// load angle 3d model
		DECLARE_CTFILENAME( fnAngle3D, "Data\\Models\\Editor\\AngleVector.mdl");
		...
		DECLARE_CTFILENAME( fnAngle3DTex, "Data\\Models\\Editor\\Vector.tex");
		m_ptdAngle3DTexture = _pTextureStock->Obtain_t( fnAngle3DTex);
		m_pAngle3DModelObject->mo_toTexture.SetData( m_ptdAngle3DTexture);

		// load bounding box
		DECLARE_CTFILENAME( fnBoundingBox, "Data\\Models\\Editor\\BoundingBox.mdl");
		...
		DECLARE_CTFILENAME( fnBoundingBoxTex, "Data\\Models\\Editor\\BoundingBox.tex");
	...
	if (cmd_strConvertList!="") {
		ConvertList(cmd_strConvertList);
		return FALSE;
	}
	...
	// if we should automaticly call preferences dialog (because WED is first time started)
	if( m_bFirstTimeStarted) OnFilePreferences();

	CTString strCmdLine=CTString(m_lpCmdLine);
...
BOOL CWorldEditorApp::SaveAllModified()
{
	...
	{
		switch(::MessageBox( pMainFrame->m_hWnd,
...
void CWorldEditorApp::ReadFromIniFileOnInit(void)
{
	...
	strcpy( strIni, GetProfileString( "World editor prefs", "Default primitive texture",
										CTFILENAME("Data\\Textures\\Editor\\Default.tex") ));
	// if exists in ini file
	if( CTString(strIni) != "")
	{
		theApp.SetNewActiveTexture( _fnmApplicationPath + CTString(strIni));
	}
	...
	INI_READ( "Distribution noise texture", CTFILENAME("Data\\Textures\\Editor\\RandomNoise.tex"));
	GET_STRING( m_fnDistributionNoiseTexture);
	
	INI_READ( "Continous noise texture", CTFILENAME("Data\\Textures\\Editor\\RandomNoise.tex"));
...
void CWorldEditorApp::ReadFromIniFile()
{
	...
	m_ccChildConfigurations[ 2].m_vpViewPrefs[ 1].m_wrpWorldRenderPrefs.SetEdgesFillType( CWorldRenderPrefs::FT_NONE);
...
void CValuesForPrimitive::WriteToIniFile(CTString strPrimitiveType)
{
	...
	SET_FLAG2( vfp_bClosed);
	INI_PRIMITIVE_WRITE( "closed");
	SET_FLAG2( vfp_bDummy);
	INI_PRIMITIVE_WRITE( "gouraud shadows");
	SET_FLAG2( vfp_bAutoCreateMipBrushes);
	...
	SET_FLAG2( vfp_bLinearStaircases);
	INI_PRIMITIVE_WRITE( "linear staircases");
	SET_FLAG2( vfp_bOuter);
...
{
	SetFPUPrecision( FPT_53BIT );				// yjpark
...
void CWorldEditorApp::WriteToIniFileOnEnd(void)
{
	...
	if( theApp.m_ptdActiveTexture != NULL)
	{
		...
		WriteProfileString( "World editor prefs", "Default primitive texture", strIni);
	}
	...
	SET_FLAG2( m_bFBMAddNegativeValues);  
	INI_WRITE( "FBM Add negative values");

	SET_FLAG2( m_bFBMRandomOffset);  
...
void CAppPrefs::WriteToIniFile()
{
	...
	SET_FLAG2( ap_CopyExistingWindowPrefs);
	INI_WRITE( "Copy existing window preferences");
	SET_FLAG2( ap_AutoMaximizeWindow);
	INI_WRITE( "Auto maximize window");
	SET_FLAG2( ap_SetDefaultColors);
	INI_WRITE( "Set default colors");
	SET_FLAG2( ap_AutomaticInfo);
	INI_WRITE( "Automatic info");
	SET_FLAG2( ap_UpdateAllways);
	INI_WRITE( "Update allways");
	SET_FLAG2( ap_BinaryGrid);
	INI_WRITE( "Binary grid");
	SET_FLAG2( ap_bSaveUndoForDelete);
	INI_WRITE( "Save undo for delete");
	SET_FLAG2( ap_bAutoColorize);
	INI_WRITE( "Auto colorize primitives");
	SET_FLAG2( ap_bShowAllOnOpen);
	INI_WRITE( "Show all on open");
	SET_FLAG2( ap_bHideShadowsOnOpen);
	INI_WRITE( "Hide shadows on open");
	SET_FLAG2( ap_bAutoUpdateDisplaceMap);
	...
	SET_FLAG2( ap_bAutoUpdateTerrainDistribution);  
...
BOOL CWorldEditorApp::LoadChildConfigurations(void)
{
	...
	try
	{
		// create binary file to recieve child configurations
...
//-----------------------------------------------------------------------------
// Purpose: DB서버 목록으로부터 정보를 얻은후, 리스트를 만듬.
// Input  : *FileName - 
// Output : Returns TRUE on success, FALSE on failure.
//-----------------------------------------------------------------------------
BOOL CWorldEditorApp::GetServerList(const char *FileName)
{
	// 서버 리스트 얻기
	FILE* pFP = fopen(FileName, "rt");
	if (pFP)
	{
		fscanf(pFP, " %d ", &m_serverlist.n);
		m_serverlist.server = new DBServerInfo[m_serverlist.n];
		int i;
		for (i = 0; i < m_serverlist.n; i++)
		{
			fgets(m_serverlist.server[i].name, 100, pFP);
			char* tmp = m_serverlist.server[i].name + strlen(m_serverlist.server[i].name) - 1;
			while (isspace(*tmp))
				*tmp-- = '\0';
			fscanf(pFP, " %s %s %s %s ",
				m_serverlist.server[i].ip,
				m_serverlist.server[i].uid,
				m_serverlist.server[i].upw,
				m_serverlist.server[i].dbname);
		}
	}
	else
	{
		AfxMessageBox(_T("There's no Server List!"));
		return FALSE;
	}
	return TRUE;
}
...
//-----------------------------------------------------------------------------
// Purpose: 서버리스트에서 DB 서버의 정보를 얻은후, 연결함.
// Input  : iIndex - 
// Output : Returns TRUE on success, FALSE on failure.
//-----------------------------------------------------------------------------
BOOL CWorldEditorApp::ConnectToDBServer(int iIndex)
{
	try
	{
		// DB 와의 연결....
		...
		m_pDBConn = new MYSQL;
		if(!mysql_init(m_pDBConn))
		{
			throw(_T("MYSQL을 초기화할수 없습니다."));
		}
		if (!mysql_real_connect (m_pDBConn,
			m_serverlist.server[iIndex].ip,
			m_serverlist.server[iIndex].uid,
			m_serverlist.server[iIndex].upw,
			m_serverlist.server[iIndex].dbname, 0, NULL, 0))
		{
			mysql_close(m_pDBConn);
			delete		m_pDBConn;
			m_pDBConn		= NULL;
			m_bDBConnected	= FALSE;
			//AfxMessageBox(_T("DB 서버에 접속할 수 없습니다."));
			throw(_T("DB 서버에 접속할 수 없습니다."));			
		}
		m_strDBName.Format( _T("%s"), m_serverlist.server[iIndex].dbname );
	}
	catch(char* strError)
	{
...
void CWorldEditorApp::SetNewActiveTexture( CTFileName fnFullTexName)
{
	...
	{
		pdtNewTexture = _pTextureStock->Obtain_t( CTFILENAME("Data\\Textures\\Editor\\Default.tex") );
		(void)err_str;
		// report error
		//AfxMessageBox( err_str);
		//return;
...
void CWorldEditorApp::OnFileOpen()
{
	...
	// world loading by seo - 40504
//강동민 수정 시작 접속 시퀀스 작업	05.31
//  _EngineGUI.FileRequester( "Choose worlds to open", FILTER_WLD FILTER_WLS FILTER_ALL FILTER_END,		// 원본.
	_EngineGUI.FileRequester( "Choose worlds to open", FILTER_WLD FILTER_WLS FILTER_ZNE FILTER_ALL FILTER_END,
		"Open world directory", "Worlds\\", "", &afnOpenedWorlds);
	FOREACHINDYNAMICARRAY( afnOpenedWorlds, CTFileName, itWorld)
	{
		// try to load document
		m_pDocTemplate->OpenDocumentFile( _fnmApplicationPath+itWorld.Current());
	}
}
...
void CWorldEditorApp::OnConvertWorlds()
{
	...
	ConvertList(fnFileList);
}

void CWorldEditorApp::ConvertList(const CTFileName &fnmList)
{
	INDEX ctLines = 0;
	char achrLine[256];
	...
	CTFileName   fnErrorFile = fnmList.NoExt() + ".err";
	// reopen list file
	fsFileList.Open_t( fnmList);
	...
	try
	{
		...
		FOREACHINDYNAMICARRAY(o3d.ob_aoscSectors, CObjectSector, itosc)
		{
			...
			FOREACHINDYNAMICARRAY(itosc->osc_aomtMaterials, CObjectMaterial, itom)
			{
				...
				{
					itom->omt_Name = "Data\\Textures\\Editor\\Default.tex";
				}
			}
		}
...
void CWorldEditorApp::OnFileNew()
{
	...
	CTFileName fnDefaultBcg = CTString(theApp.GetProfileString(
		"World editor prefs", "Default background picture",
		CTString("Data\\Textures\\Editor\\Default.tex")));
...
//!!!!    pDoc->m_woWorld.SetBackgroundTexture_t( fnDefaultBcg);
	...
	try
	{
//!!!!    pDoc->m_woWorld.SetBackgroundTexture_t( CTString("Data\\Textures\\Editor\\Default.tex"));
	}
	catch( char *strError)
	{
		AfxMessageBox( strError);
	...
	strcpy( chrColor, theApp.GetProfileString( "World editor prefs",
	...
	CString strOpenPath;
	strOpenPath = theApp.GetProfileString("World editor", "Open directory", "");
...
CTerrainLayer *GetLayer(INDEX iLayer)
{
	...
	if(!(ptTerrain->tr_ptrTerrain->tr_atlLayers.Count()>0) || iLayer>=ptTerrain->tr_ptrTerrain->tr_atlLayers.Count()) return NULL;
	return &ptTerrain->tr_ptrTerrain->tr_atlLayers[iLayer];
}
...
CTerrainLayer *GetLayer(void)
{
	CTerrain *ptTerrain=GetTerrain();
	if(ptTerrain==NULL) return NULL;
	if(!(ptTerrain->tr_ptrTerrain->tr_atlLayers.Count()>0)) return NULL;
	if(ptTerrain->tr_iSelectedLayer>=ptTerrain->tr_ptrTerrain->tr_atlLayers.Count())
	{
		ptTerrain->tr_iSelectedLayer=0;
	}
	return &ptTerrain->tr_ptrTerrain->tr_atlLayers[ptTerrain->tr_iSelectedLayer];
}
...
INDEX GetLayerIndex(void)
{
	...
	if(ptTerrain->tr_ptrTerrain->tr_atlLayers.Count()<=0 || 
		 ptTerrain->tr_iSelectedLayer>=ptTerrain->tr_ptrTerrain->tr_atlLayers.Count())
	{
		ptTerrain->tr_iSelectedLayer=0;
	}
...
void SelectLayer(INDEX iLayer)
{
	...
	if( ptrTerrain->tr_ptrTerrain->tr_atlLayers.Count() <= iLayer )
	{
		ptrTerrain->tr_iSelectedLayer = 0;
	}
	else if( iLayer < 0 )
	{
		ptrTerrain->tr_iSelectedLayer =ptrTerrain->tr_ptrTerrain->tr_atlLayers.Count()-1;
	}
	else
	{
...
// yjpark |<--
CTextureData* GetShadowMap( INDEX iShadowMap )
{
	CTerrain*	ptTerrain = GetTerrain();
	if( ptTerrain == NULL )
		return NULL;
	if( !( ptTerrain->tr_ptrTerrain->tr_aptdShadowMaps.Count() > 0 ) ||
		iShadowMap >= ptTerrain->tr_ptrTerrain->tr_aptdShadowMaps.Count() )
		return NULL;
	return ptTerrain->tr_ptrTerrain->tr_aptdShadowMaps[iShadowMap];
}

CTextureData* GetShadowMap()
{
	CTerrain*	ptTerrain = GetTerrain();
	if( ptTerrain == NULL )
		return NULL;
	if( !( ptTerrain->tr_ptrTerrain->tr_aptdShadowMaps.Count() > 0 ) )
		return NULL;
	if( ptTerrain->tr_iSelectedShadowMap >= ptTerrain->tr_ptrTerrain->tr_aptdShadowMaps.Count() )
	{
		ptTerrain->tr_iSelectedShadowMap = 0;
	}
	return ptTerrain->tr_ptrTerrain->tr_aptdShadowMaps[ptTerrain->tr_iSelectedShadowMap];
}

INDEX GetShadowMapIndex()
{
	CTerrain*	ptTerrain = GetTerrain();
	if( ptTerrain == NULL )
		return 0;
	if( ptTerrain->tr_ptrTerrain->tr_aptdShadowMaps.Count() <= 0 ||
		ptTerrain->tr_iSelectedShadowMap >= ptTerrain->tr_ptrTerrain->tr_aptdShadowMaps.Count() )
	{
		ptTerrain->tr_iSelectedShadowMap = 0;
	}
	return ptTerrain->tr_iSelectedShadowMap;
}

void SelectShadowMap( INDEX iShadowMap )
{
	CTerrain*	ptrTerrain = GetTerrain();
	if( ptrTerrain == NULL )
		return ;
	if( ptrTerrain->tr_ptrTerrain->tr_aptdShadowMaps.Count() <= iShadowMap || iShadowMap < 0 )
	{
		ptrTerrain->tr_iSelectedShadowMap = 0;
	}
	else
	{
		ptrTerrain->tr_iSelectedShadowMap = iShadowMap;
	}
}
//yjpark     -->|

//강동민 수정 시작 다중 공격 작업	08.27
int CWorldEditorApp::GetNumOfEnemies()
{
	CWorldEditorDoc *pDoc	= GetDocument();
	if(!pDoc)	return -1;
	...
	FOREACHINDYNAMICCONTAINER(pDoc->m_woWorld.wo_cenAllEntities, CEntity, iten)
	{
		if(iten->IsEnemySpawner())
		{
			...
			{
				iCount		+= ENTITYPROPERTY( &*iten, epPropertyCount.ep_slOffset, INDEX);
			}
		}
	}
	return iCount;
