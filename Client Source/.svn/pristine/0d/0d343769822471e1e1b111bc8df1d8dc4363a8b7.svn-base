...
map<string, WinTrace*>* TTrace::m_winTraceList = new map<string, WinTrace*>;
map<string, WinWatch*>* TTrace::m_winWatchList = new map<string, WinWatch*>;
...
UINT_PTR           TTrace::m_socketHandle = (SOCKET)-1;
bool               TTrace::m_ExecuteViewer = true;
...
public:
	MicroTime()
		{
		time(&m_startTime);
		QueryPerformanceFrequency((LARGE_INTEGER*)&m_llFreq);
		QueryPerformanceCounter((LARGE_INTEGER*)&m_llStart);
		}

#ifdef GetCurrentTime
#undef GetCurrentTime
#endif
	inline bool GetCurrentTime(LPSYSTEMTIME pSystemTime, LPINT pMicroSec)
		{
		if (!QueryPerformanceCounter((LARGE_INTEGER*)&m_llCurrent))
			return false;

		double   gap  = (double)(m_llCurrent - m_llStart) / (double)m_llFreq;
		LONGLONG sec  = (INT)gap;
		LONGLONG usec = (INT)((gap - sec) * 1000000);
		time_t   t    = m_startTime + sec;
		LONGLONG ll   = Int32x32To64(t, 10000000) + 116444736000000000;
		FILETIME ft   = { (DWORD)ll, (DWORD)(ll >> 32) };

		FILETIME lft;
		if (!FileTimeToLocalFileTime(&ft, &lft))
			return false;

		if (!FileTimeToSystemTime(&lft, pSystemTime))
			return false;
...
private:
	time_t m_startTime;
	LONGLONG m_llFreq, m_llStart, m_llCurrent;
};
...
DWORD TTrace::Init(void)
	...
	MessageReady = CreateEvent(NULL, TRUE, FALSE, NULL);
	...
	isStopping = false;
	hThread = (HANDLE)_beginthreadex(NULL, 0, &MsgThread, NULL, 0, NULL);
...
void TTrace::Stop(void)
	...
	WaitForSingleObject(ClosedEvent, 1000); // wait 1s max for the closed event
	...
	// delete m_winTraceList content and array
	map<string, WinTrace*>::const_iterator tb = m_winTraceList->begin();
	map<string, WinTrace*>::const_iterator te = m_winTraceList->end();
	for (; tb != te; tb++)
		{
		WinTrace* wintrace = tb->second;
		delete wintrace;
		}
	delete m_winTraceList;
	m_winTraceList = NULL;

	// delete m_winWatchList content and array
	map<string, WinWatch*>::const_iterator wb = m_winWatchList->begin();
	map<string, WinWatch*>::const_iterator we = m_winWatchList->end();
	for (; wb != we; wb++)
		{
		WinWatch* winwatch = wb->second;
		delete winwatch;
		}
	delete m_winWatchList;
	m_winWatchList = NULL;
...
void TTrace::Flush(int FlushTimeOut)
	...
	// create the event
	HANDLE flushEvent = CreateEvent(NULL, TRUE, FALSE, NULL);
...
char* TTrace::CreateTraceID()
	...
	DWORD Data1 = fileTime.dwLowDateTime;

	//4-5    The middle field of the timestamp.
	WORD Data2 = LOWORD(fileTime.dwHighDateTime);

	//6-7    The high field of the timestamp multiplexed
	//       with the version number.
	//       Version number is 0x0002
	WORD Data3 = (HIWORD(fileTime.dwHighDateTime) & 0xFFF0) | 0x0002;

	//8-9    Here we store sequence number
	m_ClockSequenceBase++;

	WORD Sequence = LOWORD(m_ClockSequenceBase);
	...
	char* bufferId = (char*)malloc(32+1);

	#if defined(_MSC_VER) && (_MSC_VER >= 1400) && (!UNDER_CE)
	sprintf_s(bufferId, 33, "%08x%04x%04x%04x%04x%04x%04x", Data1, Data2, Data3, Sequence,m_DeviceId1,m_DeviceId2,m_DeviceId3);
	#else
	_snprintf(bufferId, 33, "%08x%04x%04x%04x%04x%04x%04x", Data1, Data2, Data3, Sequence,m_DeviceId1,m_DeviceId2,m_DeviceId3);
	#endif
	return bufferId;
	}
...
void TTrace::SendToWinWatchClient(CommandList* Commands, const char* winWatchId)
	...
	// message time
	int nUSec = -1;
	if (!g_time.GetCurrentTime(&Time, &nUSec))
		GetLocalTime(&Time);
	...
	if (nUSec == -1)
		...
		if (TTrace::Options()->SendDate)
			#if defined(_MSC_VER) && (_MSC_VER >= 1400) && (!UNDER_CE)
			sprintf_s(buffer, MAX_PATH, "%04d%2d%2d %02d:%02d:%02d:%03d", Time.wYear, Time.wMonth, Time.wDay, Time.wHour, Time.wMinute, Time.wSecond, Time.wMilliseconds);
			#else
			_snprintf(buffer, MAX_PATH, "%04d%2d%2d %02d:%02d:%02d:%03d", Time.wYear, Time.wMonth, Time.wDay, Time.wHour, Time.wMinute, Time.wSecond, Time.wMilliseconds);
			#endif
		else
			#if defined(_MSC_VER) && (_MSC_VER >= 1400) && (!UNDER_CE)
			sprintf_s(buffer, MAX_PATH, "%02d:%02d:%02d:%03d", Time.wHour, Time.wMinute, Time.wSecond, Time.wMilliseconds);
			#else
			_snprintf(buffer, MAX_PATH, "%02d:%02d:%02d:%03d", Time.wHour, Time.wMinute, Time.wSecond, Time.wMilliseconds);
			#endif
		}
	else
		{
		if (TTrace::Options()->SendDate)
			#if defined(_MSC_VER) && (_MSC_VER >= 1400) && (!UNDER_CE)
			sprintf_s(buffer, MAX_PATH, "%04d%2d%2d %02d:%02d:%02d:%06d", Time.wYear, Time.wMonth, Time.wDay, Time.wHour, Time.wMinute, Time.wSecond, nUSec);
			#else
			_snprintf(buffer, MAX_PATH, "%04d%2d%2d %02d:%02d:%02d:%06d", Time.wYear, Time.wMonth, Time.wDay, Time.wHour, Time.wMinute, Time.wSecond, nUSec);
			#endif
		else
			#if defined(_MSC_VER) && (_MSC_VER >= 1400) && (!UNDER_CE)
			sprintf_s(buffer, MAX_PATH, "%02d:%02d:%02d:%06d", Time.wHour, Time.wMinute, Time.wSecond, nUSec);
			#else
			_snprintf(buffer, MAX_PATH, "%02d:%02d:%02d:%06d", Time.wHour, Time.wMinute, Time.wSecond, nUSec);
			#endif
		}
...
void TTrace::SendToWinTraceClient(CommandList* Commands, const char* winTraceId, const char* dateTime, const char* threadName)
	...
	if (winTraceId != NULL && winTraceId[0])
		...
		switch (ch)
			...
			default:
				if ((ch < 32) || ((ch >= 160) && (ch < 256)))  // 0 to 31 and 160 to 256
					...
					#else
					_snprintf(line, 20, "&#%u;", unsigned int(ch));
					#endif
		...
		if (m_options->sendMode == Socket)
			...
			if (m_IsSocketInitialized == false)
				...
				if (m_serverSockAddr.sin_addr.s_addr == INADDR_NONE)
					...
					hostent* lphost = gethostbyname(m_options->socketHost); // lphost is allocated by Windows Sockets
				...
				m_serverSockAddr.sin_port = htons((WORD)m_options->socketPort);       // port to network port
			...
			#else
			_snprintf(buffer, 100, "%d", j);
			#endif

			send(m_socketHandle, buffer, (int)strlen(buffer)+1, 0);

			// send the buffer
			send(m_socketHandle, line, (int)j, 0);
			}
...
int TTrace::ParseForInternal(CommandList* Commands)
	...
	for (c1_elem = Commands->begin(); c1_elem < c1_end; c1_elem++)
		...
		switch (command)
			...
			case CST_THREAD_ID:
				...
				#else
				_snprintf(temp, 20, "0x%X", intParam);
				#endif
				ThreadID = temp;
				break;
	...
	if (TraceForm->LogFileType == 3)             // 3, Local log disabled.
		...
		if (TraceForm->CurrentFileNumber != 0)
			...
			#else
			_snprintf(temp, 20, "%d", TraceForm->CurrentFileNumber);
			#endif
		...
		#else
		_snprintf(temp, 20, "%04d%02d%02d", Time.wYear, Time.wMonth, Time.wDay);
		#endif
		TraceForm->LastLocalLogFileName.append(temp);
	...
	if (isOpen == false)
		...
		if (TraceForm->IsMultiColTree)
			...
			#else
			_snprintf(temp, 20, "%d", TraceForm->MainCol);
			#endif
		...
		xml.insert(0,"<?xml version='1.0' encoding='windows-1252'?>\n<Data>\n"); // ISO-8859-1, ASCII, ISO-8859-15, windows-1252
		}
...
WinTrace* TTrace::GetWinTrace(const char* winTraceId)
	{
	WinTrace* winTrace;
#ifdef USE_SINGLE_TAB
	if (m_winTraceList->empty())
		{
		SYSTEMTIME st;
		GetLocalTime(&st);
		char szTraceId[256];
		#if defined(_MSC_VER) && (_MSC_VER >= 1400) && (!UNDER_CE)
		sprintf_s(szTraceId, 256, "Trace - %02d%02d%02d_%02d%02d%02d", st.wYear%100, st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond);
		#else
		_snprintf(szTraceId, 256, "Trace - %02d%02d%02d_%02d%02d%02d", st.wYear%100, st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond);
		#endif
		winTrace = new WinTrace(szTraceId, szTraceId);
		m_winTraceList->insert(make_pair(szTraceId, winTrace));
		}
	else
		{
		map<string, WinTrace*>::const_iterator it = m_winTraceList->begin();
		winTrace = it->second;
		}
#else
	if (winTraceId == NULL)
		...
		{
		map<string, WinTrace*>::const_iterator it = m_winTraceList->find(winTraceId);
		if (it == m_winTraceList->end())
			{
			winTrace = new WinTrace(winTraceId, winTraceId);
			m_winTraceList->insert(make_pair(winTraceId, winTrace));
			}
		else
			{
			winTrace = it->second;
...
WinWatch* TTrace::GetWinWatch(const char* winWatchId)
	{
	WinWatch* winWatch;
	map<string, WinWatch*>::const_iterator it = m_winWatchList->find(winWatchId);
	if (it == m_winWatchList->end())
		{
		winWatch = new WinWatch(winWatchId, winWatchId);
		m_winWatchList->insert(make_pair(winWatchId, winWatch));
		}
	else
		{
		winWatch = it->second;
...
void TTrace::SendToWinTrace(const char* winTraceId, const char* leftMsg, const char* rightMsg/* = NULL*/)
	{
	GetWinTrace(winTraceId)->Debug()->Send(leftMsg, rightMsg);
	}

void TTrace::SendToWinWatch(const char* winWatchId, const char* watchName, const char* watchValue)
	{
	GetWinWatch(winWatchId)->Send(watchName, watchValue);
	}

void TTrace::SendToWinTraceDump(const char* winTraceId, const char* title, const char* buf, unsigned int bufLen, const char* rightMsg/* = NULL*/)
	{
	GetWinTrace(winTraceId)->Debug()->SendDump(title, rightMsg, title, buf, bufLen);
	}
...
TraceNodeEx* TraceNodeBase::CreateChildEx(const char* leftMsg /* = NULL */, const char* rightMsg /* = NULL */)
	{
	TraceNodeEx* Node = new TraceNodeEx(this);   // Node->id is generated (GUID)
	...
	if (m_FontDetails != NULL)
		...
		for (FontBegin = m_FontDetails->begin(); FontBegin < FontEnd; FontBegin++)
			...
			if (font->FontName.empty())
				...
				#else
				_snprintf(message, 33, "%5d%3d%1d%1d%11d%11d", CST_MEMBER_FONT_DETAIL,font->ColId,bold,italic,font->Color,font->Size);
				#endif
				}
			else
				{
				...
				#else
				_snprintf(message, MsgLen, "%5d%3d%1d%1d%11d%11d%s", CST_MEMBER_FONT_DETAIL,font->ColId,bold,italic,font->Color,font->Size,fontName);
				#endif
				}
...
TMemberNode* TMemberNode::Add(const char* strCol1, const char* strCol2 /* = NULL */, const char* strCol3 /* = NULL */)
	{
	TMemberNode* member = new TMemberNode(strCol1, strCol2, strCol3);
	Add(member);
	return member;
	}
...
const char* TraceOptions::CheckProcessName(void)
	...
	if (m_processFileName == NULL)
		...
		const char* ptr = pmbFilename + nbChar -1;
...
WinWatch::WinWatch(const char* WinWatchID, const char* WinWatchText)
	...
	if (WinWatchID == NULL || WinWatchID[0] == 0)
		{
		char* c = TTrace::CreateTraceID();
		id = c; //copy
		free(c);
		}
	else
	...
	if (WinWatchText == NULL || WinWatchText[0] == 0)
...
WinWatch::WinWatch(const wchar_t* WinWatchID, const wchar_t* WinWatchText)
	...
	if (strWinWatchID == NULL || strWinWatchID[0] == 0)
		{
		char* c = TTrace::CreateTraceID();
		id = c; //copy
		free(c);
	...
	if (strWinWatchText == NULL || strWinWatchText[0] == 0)
...
WinTrace::WinTrace(const char* WinTraceID, const char* WinTraceTitle)
	{
	if (WinTraceID == NULL || WinTraceID[0] == 0)
		{
		char* c = TTrace::CreateTraceID();
		id = c; //copy
		free(c);
	...
	if (WinTraceID != NULL && strcmp(WinTraceID, "_") == 0)
		return;  // don't create new window on the viewer

	// create the trace window
	CommandList* Commands = new CommandList();

	if (WinTraceTitle == NULL || WinTraceTitle[0] == 0)
...
WinTrace::WinTrace(const wchar_t* WinTraceID, const wchar_t* WinTraceTitle)
	...
	if (strWinTraceID == NULL || strWinTraceID[0] == 0)
		{
		char* c = TTrace::CreateTraceID();
		id = c; //copy
		free(c);
	...
	if (strWinTraceID != NULL && strcmp(strWinTraceID, "_") == 0)
		{
		...
		if (strWinTraceTitle == NULL || strWinTraceTitle[0] == 0)
			Commands->Add(CST_TREE_NAME, id.c_str());
		else
...
/// <param name="FileName">file to open</param>
/// <param name="Mode">Local and viewer site log mode.</param>
/// <param name="MaxLines">Number of lines before starting a new file (default : -1).</param>
...
void TraceToSend::UnIndent(const wchar_t* leftMsg, const wchar_t* rightMsg, int BackGroundColor, bool isExit)
	...
	if (leftMsg != NULL || rightMsg != NULL)
		...
		char* c = TTrace::CreateTraceID();   // then give new ID
		node->id = c; //copy
		free(c);
...
void TraceToSend::SendXml(const char* leftMsg, const char* xml)
	...
	TraceNodeEx* Node = new TraceNodeEx(this, true);
	Node->leftMsg = leftMsg;
	Node->AddXml(xml);
	Node->Send();
	delete Node;
...
/// <param name="byteCount"></param>
/// <returns>itself</returns>
TraceNodeEx* TraceNodeEx::AddDump(const char* Title, const char* memory, const unsigned, const unsigned byteCount)
	{
	...
	TMemberNode* DumpGroup = new TMemberNode(Title);
	...
	while (byteDumped < byteCount)
		...
		while ((byteDumped < byteCount) && (d < 16))
			...
			#else
			_snprintf(ptrHex, 4, "%02X ", OneChar);
			#endif
			ptrHex += 3;
		...
		#else
		_snprintf(adress, 8, "%06X", beginLine);
		#endif
		DumpGroup->Add(adress, hexa_representation);
		}
	...
	#else
	_snprintf(hexa_representation, 50, "%d byte(s) dumped", byteDumped);
	#endif
...
TraceNode* TraceNodeEx::Send(void)
	...
	CommandList* Commands = new CommandList();              // Commands to send
	...
	if (m_FontDetails != NULL)
		...
		for (FontBegin = m_FontDetails->begin(); FontBegin < FontEnd; FontBegin++)
			...
			if (font->FontName.compare("BackgroundColor") == 0)
				...
				#else
				_snprintf(message, 28, "%5d%11d%11d", CST_BACKGROUND_COLOR,font->Color,font->ColId); // param : color, colId
				#endif
				}
				...
				#else
				_snprintf(message, 33, "%5d%3d%1d%1d%11d%11d", CST_FONT_DETAIL,font->ColId,bold,italic,font->Color,font->Size);
				#endif
				}
			else
				{
				...
				#else
				_snprintf(message, MsgLen, "%5d%3d%1d%1d%11d%11d%s", CST_FONT_DETAIL,font->ColId,bold,italic,font->Color,font->Size,fontName);
				#endif
				}
			Commands->push_back(message);
			free(message);
...
TraceNode* TraceNode::SetBackgroundColor(const int color, const int colId)
	...
	#else
	_snprintf(StrColId, 12, "%11d", colId);                      //  11 + 1
	#endif
...
TraceNode* TraceNode::SetFontDetail(const int colId, const bool bold, const bool italic, const int color, const int size, const char* fontName)
	...
	if (fontName == NULL)
		...
		#else
		_snprintf(message, 33, "%5d%3d%1d%1d%11d%11d", CST_FONT_DETAIL, colId, int_bold, int_italic, color, size);
		#endif
		}
	else
		{
		...
		#else
		_snprintf(message, MsgLen, "%5d%3d%1d%1d%11d%11d%s", CST_FONT_DETAIL, colId, int_bold, int_italic, color, size, fontName);
		#endif
		}
