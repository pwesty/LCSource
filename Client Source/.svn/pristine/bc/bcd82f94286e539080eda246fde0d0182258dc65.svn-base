...
static int _iShadowMapShift = 0;
static int _iShadingMapShift = 1;
static int _iShadowMapOverbright = 100;
static int _pixGlobalPretenderTextureWidth = 64;
	...
	: CDialog(CDlgTerrainProperties::IDD, pParent)
{
	...
	_iShadowMapShift = TR_GetShadowMapAspect( ptrTerrain );
	_iShadingMapShift = TR_GetShadingMapAspect( ptrTerrain );
	_iShadowMapOverbright = ptrTerrain->tr_ptrTerrain->tr_fShadowOverbright * 100;
	_pixGlobalPretenderTextureWidth = TR_GetTopMapWidth( ptrTerrain );
}
...
void CDlgTerrainProperties::DoDataExchange(CDataExchange* pDX)
{
	...
	{
		m_fTerrainWidth = TR_GetTerrainSize( ptrTerrain )(1);
		m_fTerrainHeight = TR_GetTerrainSize( ptrTerrain )(2);
		m_fTerrainLength = TR_GetTerrainSize( ptrTerrain )(3);
		...
		PIX		pixSwW = Clamp( PIX(( pixW - 1 ) * pow( 2.f, iShadowMapShift )), PIX(4), PIX(2048) );
		PIX		pixSwH = Clamp( PIX(( pixH - 1 ) * pow( 2.f, iShadowMapShift )), PIX(4), PIX(2048) );
		...
		INDEX	iShadowMapOverbright = m_ctrlShadowMapOverbrightSlider.GetPos();
		strTemp.PrintF( "%d %%", iShadowMapOverbright );

		INDEX	iShadingMapShift = 10 - m_ctrlShadingMapSlider.GetPos();
		PIX		pixSdW = Clamp( PIX(pixSwW * pow( 2.f, -iShadingMapShift )), PIX(4), PIX(pixSwW) );
		PIX		pixSdH = Clamp( PIX(pixSwH * pow( 2.f, -iShadingMapShift )), PIX(4), PIX(pixSwH) );
		...
		INDEX	pixTopMapWidth = m_ctrlGlobalPretenderTexture.GetItemData( iGlobalPretender );
		strTemp.PrintF( "Size: %d x %d", pixTopMapWidth, pixTopMapWidth );
		...
		strTemp.PrintF("Terrain total uncompressed static data memory consumption (without tile pretender textures and geometry): %d KB",
						...
						*/
		m_strMemoryConsumption="";

		m_fLODSwitch = ptrTerrain->tr_ptrTerrain->tr_fDistFactor;
		m_fFirstTMLOD = ptrTerrain->tr_ptrTerrain->tr_iFirstTopMapLOD;
	}

	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CDlgTerrainProperties)
	DDX_Control(pDX, IDC_SHADOW_MAP_OVERBRIGHT, m_ctrlShadowMapOverbrightSlider);
	...
	DDX_Text(pDX, IDC_TERRAIN_FIRST_TOPMAP_LOD, m_fFirstTMLOD);
	DDV_MinMaxFloat(pDX, m_fFirstTMLOD, -1.0f, 1.e+007f);
	...
	DDX_Text(pDX, IDC_SHADOW_MAP_OVERBRIGHT_T, m_strShadowMapOverbright);
	//}}AFX_DATA_MAP
	...
	if( pDX->m_bSaveAndValidate != FALSE )
	{
		...
		INDEX	iItemH = m_ctrlHeightMapHeight.GetCurSel();
		PIX		pixH = m_ctrlHeightMapHeight.GetItemData( iItemH );
		if( TR_GetHeightMapWidth( ptrTerrain ) != pixW ||
			TR_GetHeightMapHeight( ptrTerrain ) != pixH )
		{
			TR_SetHeightMapSize( ptrTerrain, pixW, pixH );
			TR_UpdateTopMap( ptrTerrain );
			bUpdateTerrain = TRUE;
		}

		// handle terrain size change
		if( TR_GetTerrainSize( ptrTerrain )( 1 ) != m_fTerrainWidth ||
			TR_GetTerrainSize( ptrTerrain )( 2 ) != m_fTerrainHeight ||
			TR_GetTerrainSize( ptrTerrain )( 3 ) != m_fTerrainLength )
		{
			TR_SetTerrainSize( ptrTerrain, FLOAT3D( m_fTerrainWidth, m_fTerrainHeight, m_fTerrainLength ) );
			bUpdateTerrain=TRUE;
		}
		...
		if( ctQuads != ptrTerrain->tr_ptrTerrain->tr_ctQuadsPerTileX )
		{
			ptrTerrain->tr_ptrTerrain->tr_ctQuadsPerTileX = ctQuads;
			bUpdateTerrain = TRUE;
		}
    
		// handle LOD distance change
		if( m_fLODSwitch != ptrTerrain->tr_ptrTerrain->tr_fDistFactor )
		{
			TR_SetDistanceFactor( ptrTerrain, m_fLODSwitch );
			bUpdateTerrain = TRUE;
		}

		// handle first top map lod
		if( m_fFirstTMLOD != ptrTerrain->tr_ptrTerrain->tr_iFirstTopMapLOD )
			ptrTerrain->tr_ptrTerrain->tr_iFirstTopMapLOD = m_fFirstTMLOD;

		// handle global pretender texture change
		INDEX	iGlobalPretender = m_ctrlGlobalPretenderTexture.GetCurSel();
		INDEX	pixTopMapWidth = m_ctrlGlobalPretenderTexture.GetItemData( iGlobalPretender );
		if( pixTopMapWidth != TR_GetTopMapWidth( ptrTerrain ) )
			TR_SetTopMapSize( ptrTerrain, pixTopMapWidth, pixTopMapWidth );
			...
			( m_ctrlShadingMapSlider.GetPos() != 10 - _iShadingMapShift ) )
		{
			...
			Matrix12	mTerrain;
			TR_SetShadowMapSize( ptrTerrain, _iShadowMapShift, _iShadingMapShift );
			TR_GetMatrixFromEntity( mTerrain, ptrTerrain );
			TR_UpdateShadowMap( ptrTerrain, mTerrain, FLOATaabbox3D() );
			// bUpdateTerrain=TRUE;
		}

		if( m_ctrlShadowMapOverbrightSlider.GetPos() != _iShadowMapOverbright )
		{
			_iShadowMapOverbright = m_ctrlShadowMapOverbrightSlider.GetPos();
			ptrTerrain->tr_ptrTerrain->tr_fShadowOverbright = _iShadowMapOverbright / 100.0f;
			Matrix12	mTerrain;
			TR_GetMatrixFromEntity( mTerrain, ptrTerrain );
			TR_UpdateShadowMap( ptrTerrain, mTerrain, FLOATaabbox3D() );
		}
...
void CDlgTerrainProperties::InitComboBoxes(void)
{
	...
	INDEX	iToSelect = 3;
	for( INDEX iQuads = 4; iQuads <= 128; iQuads *= 2 )
	{
		CTString	strItem;
		strItem.PrintF( "%d x %d", iQuads, iQuads );
		INDEX iAddedAs = m_ctrlQuadsPerTile.AddString( strItem );
		m_ctrlQuadsPerTile.SetItemData( iAddedAs, iQuads );
		if( ptrTerrain->tr_ptrTerrain->tr_ctQuadsPerTileX == iQuads )
		{
			iToSelect = iAddedAs;
		}
	}
	...
	for( iWidth = 32; iWidth <= 2048; iWidth *= 2 )
	{
		...
		// width
		INDEX	iW = m_ctrlHeightMapWidth.AddString( strItem );
		m_ctrlHeightMapWidth.SetItemData( iW, iWidth + 1 );
		if( TR_GetHeightMapWidth( ptrTerrain ) == iWidth + 1 )
			iToSelectW = iW;
		// height
		INDEX	iH = m_ctrlHeightMapHeight.AddString( strItem );
		m_ctrlHeightMapHeight.SetItemData( iH, iWidth + 1 );
		if( TR_GetHeightMapHeight( ptrTerrain ) == iWidth + 1 )
			iToSelectH=iH;
	}
...
void CDlgTerrainProperties::PrepareTilePretenderCombo(void)
{
	...
	for( INDEX iWidth = 4; iWidth <= 2048; iWidth *= 2 )
	{
		...
		INDEX	iAddedAs = m_ctrlTilePretender.AddString( strItem );
...
void CDlgTerrainProperties::PrepareGlobalPretenderCombo(void)
{
	...
	const PIX	pixTopMapWidth = TR_GetTopMapWidth( ptrTerrain );
	const PIX	pixTopMapHeight = TR_GetTopMapHeight( ptrTerrain );

	INDEX	iSelectedItem = -1;
	m_ctrlGlobalPretenderTexture.ResetContent();
	for( PIX pixWidth = 16; pixWidth <= 2048; pixWidth *= 2 )
	{
		CTString	strItem( 0, "%d x %d", pixWidth, pixWidth );
		INDEX	iAddedItem = m_ctrlGlobalPretenderTexture.AddString( (const char *)strItem );
		m_ctrlGlobalPretenderTexture.SetItemData( iAddedItem, pixWidth );
		if( pixTopMapWidth == pixWidth )
			iSelectedItem = iAddedItem;
	}
	m_ctrlGlobalPretenderTexture.SetCurSel( iSelectedItem );
	...
	for( INDEX iWidth = 4; iWidth <= 2048; iWidth *= 2 )
	{
		...
		INDEX	iAddedAs = m_ctrlGlobalPretenderTexture.AddString( strItem );
...
BOOL CDlgTerrainProperties::OnInitDialog() 
{
	...
	m_ctrlShadowMapOverbrightSlider.SetRange( 0, 100, TRUE );
	...
	m_ctrlShadowMapOverbrightSlider.SetPos( _iShadowMapOverbright );
	m_ctrlShadowMapOverbrightSlider.Invalidate( FALSE );
