...
public:
	...
	eExtenUndo					m_eExUndo;
	CWorldEditorView*			m_pView;
	CPlacement3D				m_plMouseMove;
	std::vector<CEntity*>		m_vecSelectedEntity;
	//std::vector<CPlacement3D> m_vecSelectedEntityPlacements;
	//CEntitySelection m_vecSelectedEntity;
	CStaticArray<CPlacement3D>	m_vecSelectedEntityPlacements;

	std::vector<CBrushVertex*>	m_vecSelectedVertex;
	CStaticArray<DOUBLE3D>		m_avStartDragVertices;

	CExtenUndo( CWorldEditorView* pView, CPlacement3D& plMouseMove, CEntitySelection& selEntitySelection, CStaticArray<CPlacement3D>& aSelectedEntityPlacements);
	CExtenUndo( CWorldEditorView* pView, CEntitySelection& selEntitySelection);
	CExtenUndo( CWorldEditorView* pView, CStaticArray<DOUBLE3D>& avStartDragVertices, CBrushVertexSelection& selVertexSelection);
	CExtenUndo( const CExtenUndo& cUndo);

	BOOL HaveEntity( CEntitySelection& selEntitySelection );
};
...
public:
  ...
  BOOL			m_bOpenedZoneFile;
  char			m_szWorldFileName[255];
  ...
  //Su-won
  std::vector<CExtenUndo> m_ExUndo;
...
public:
  ...
  void ClearSelections(ESelectionType stExcept=ST_NOSEL);
...
public:
	...
	afx_msg void OnUpdateUndoCheckOut(CCmdUI* pCmdUI);
	afx_msg void OnGetLatestVersion();
	afx_msg void OnUpdateGetLatestVersion(CCmdUI* pCmdUI);
	...
	afx_msg void OnExportBspattributemap();
	afx_msg void OnCreateWater();
	afx_msg void OnExportBspheightmap();
	afx_msg void OnExportSvrHeightmap();
	afx_msg void OnGenerateSvrHeightmap();
	afx_msg void OnImportAttributemap();
	afx_msg void OnExportAttributemap();
	afx_msg void OnGenerateAttributemap();
	afx_msg void OnClearAttributemap();
	afx_msg void OnEditExtenundo();
	afx_msg void OnChangeAttributemapDepthtest();
	afx_msg void OnDrawModelattribute();
	//}}AFX_MSG
	DECLARE_MESSAGE_MAP()
};
