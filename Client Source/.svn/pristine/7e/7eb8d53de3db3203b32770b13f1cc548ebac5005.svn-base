...
#if CXIMAGE_SUPPORT_LC_TEX
	if (imagetype==CXIMAGE_FORMAT_LC_TEX){
		CxImageLCTEX newima;
		newima.Ghost(this);
		if (newima.Encode(hFile)){
			return true;
...
#endif
#if CXIMAGE_SUPPORT_LC_TEX
		{ CxImageLCTEX newima; newima.CopyInfo(*this); if (newima.Decode(hFile)) { Transfer(newima); return true; } else hFile->Seek(pos,SEEK_SET); }
#endif
	}
